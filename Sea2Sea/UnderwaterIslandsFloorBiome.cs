...
public class UnderwaterIslandsFloorBiome : CustomBiome {
	...
	public override void register() {
		...
		this.createDiscoveryStoryGoal(0, SeaToSeaMod.miscLocale.getEntry("glassforestenter"));
	...
	public override bool isInBiome(Vector3 pos) {
		...
		return this.isInBiome(orig, pos);
	}
	...
	public float getTemperatureBoost(float baseline, Vector3 pos) {
		...
		if (boost <= 0)
			return 0;
		boost /= 1 + ((float)this.getDistanceToBiome(pos) * 0.01F);
