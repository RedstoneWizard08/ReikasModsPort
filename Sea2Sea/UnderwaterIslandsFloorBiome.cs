...
public class UnderwaterIslandsFloorBiome : CustomBiome {
    ...
    public static readonly Vector3 biomeCenter = new(-107, -481, 953);

    public static readonly float waterTemperature = 35;

    public static readonly Vector3 wreckCtrPos1 = new(-110.76F, -499F, 940.19F);
    public static readonly Vector3 wreckCtrPos2 = new(-138.38F, -497F, 932.69F);

    // public static Color? waterColor;
    public static readonly Vector3 waterColorFalloff = new(4, 10F, 2.0F); //new Vector3(5, 12.3F, 2.5F);
    public static readonly float murkiness = 1.75F; //1.4F;
    public static readonly float fogStart = 0;

    public static readonly UnderwaterIslandsFloorBiome instance = new();

    private readonly GlassForestAtmoFX atmoFX = new();
    ...
    public override void register() {
        ...
        GenUtil.registerWorldgen(
            new PositionedPrefab(VanillaCreatures.GHOST_LEVIATHAN.prefab, new Vector3(-125, -450, 980))
        );
        ...
        */
        ...
        GenUtil.registerPrefabWorldgen(
            ...
            LargeWorldEntity.CellLevel.Batch,
    ...
    public override VanillaMusic[] getMusicOptions() {
        return [VanillaMusic.ILZ, VanillaMusic.JELLYSHROOM, VanillaMusic.AURORA];
    }
    ...
    public override bool IsInBiome(Vector3 pos) {
        var orig = WaterBiomeManager.main.GetBiome(pos, false);
    ...
    public override double getDistanceToBiome(Vector3 vec) {
        var ret = Math.Max(0, Vector3.Distance(vec, biomeCenter) - biomeRadius);
    ...
    public float getTemperatureBoost(float baseline, Vector3 pos) {
        var boost = (-pos.y - minimumDepth - 75) * 0.5F; // so add about 50C
        if (boost <= 0)
            return 0;
        boost /= 1 + (float)getDistanceToBiome(pos) * 0.01F;
        var ret = Mathf.Min(boost, 200 - baseline);
        var dist = (float)MathUtil.getDistanceToLineSegment(pos, wreckCtrPos1, wreckCtrPos2);
        if (dist <= 40) {
...
internal class GlassForestAtmoFX : GenUtil.CustomPrefabImpl {
    [SetsRequiredMembers]
