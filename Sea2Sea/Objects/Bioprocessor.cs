...
public class Bioprocessor : CustomMachine<BioprocessorLogic> {
    ...
    internal static readonly SoundManager.SoundData workingSound = SoundManager.registerSound(
        ...
        s => {
            workingSoundReference = s;
            SoundManager.setup3D(s, 16);
            SoundManager.setLooping(s);
        }
    );
    ...
    public static void addRecipes() {
        ...
        addRecipe(
            ...
            CraftingItems.getItem(CraftingItems.Items.WeakAcid).Info.TechType,
            2,
            2,
            30,
            5,
        ...
        addRecipe(
            new TypeInput(C2CItems.alkali),
            CraftingItems.getItem(CraftingItems.Items.Sealant).Info.TechType,
            4,
            30,
            600,
            5
        ...
        addRecipe(
            new TypeInput(TechType.GasPod),
            CraftingItems.getItem(CraftingItems.Items.Chlorine).Info.TechType,
            1,
            15,
            240,
            3,
        ...
        addRecipe(
            new TypeInput(TechType.SnakeMushroomSpore),
            CraftingItems.getItem(CraftingItems.Items.Luminol).Info.TechType,
            2,
            90,
            hard ? 2500 : 1500,
            2
        ...
        addRecipe(
            new TypeInput(TechType.HatchingEnzymes),
            CraftingItems.getItem(CraftingItems.Items.SmartPolymer).Info.TechType,
            6,
            120,
            hard ? 4500 : 3000,
            2
        ...
        addRecipe(
            new TypeInput(TechType.SeaTreaderPoop),
            CraftingItems.getItem(CraftingItems.Items.TreaderEnzymes).Info.TechType,
            1,
            5,
            120,
            1,
    ...
    private static TechType createRecipeDelegate(BioRecipe r, bool preventUnlock = false) {
        ...
        RecipeUtil.addIngredient(item.Info.TechType, r.inputItem.getIngredientDisplay(), r.inputCount);
    ...
    public static TechType getRecipeReferenceItem(TechType input) {
        return delegates[input].Info.TechType;
    }
...
public class BioprocessorLogic : DiscreteOperationalMachineLogic {
    ...
    protected override void updateEntity(float seconds) {
        ...
        if (seconds <= 0)
            return;
        storage.container.isAllowedToRemove += (pp, vb) =>
            currentOperation == null ||
            (pp.GetTechType() != CraftingItems.getItem(CraftingItems.Items.BioEnzymes).Info.TechType &&
             pp.GetTechType() != currentOperation.inputItem.getBaseType());
        storage.container.isAllowedToAdd += (pp, vb) =>
            currentOperation == null || pp.GetTechType() == currentOperation.outputItem || pp.GetTechType() ==
            CraftingItems.getItem(CraftingItems.Items.KelpEnzymes).Info.TechType;
        if (!setCollision) {
        ...
        if (has != had) {
            SoundManager.playSoundAt(
                SoundManager.buildSound(
                    r == null ? "event:/sub/seamoth/seamoth_light_off" : "event:/sub/seamoth/seamoth_light_on"
                ),
                gameObject.transform.position
            );
...
public sealed class TypeInput : BioInput {
    ...
    internal TypeInput(BasicCustomPlant plant) : this(plant.seed.Info.TechType) {
    }

    internal TypeInput(CustomPrefab pfb) : this(pfb.Info.TechType) {
    }
...
sealed class SparklePeeperInput : BioInput {
    ...
    public override TechType getIngredientDisplay() {
        return Bioprocessor.sparklePeeperDisplay.Info.TechType;
    }
...
public class BioRecipe {
    ...
    public override string ToString() {
        return string.Format(
            "[BioRecipe InputItem={0}, OutputItem={1}, enzyCount={2}, ProcessTime={3}, secondsPerEnzyme={4}, InputCount={5}, OutputCount={6}, totalEnergyCost={7}, powerPerSecond={8}]",
            inputItem,
            outputItem,
            enzyCount,
            processTime,
