...
public class FallingGlassForestWreck : CustomPrefab {
    ...
    internal static void cleanup(GameObject world) {
        ...
        GameObject exterior = world.getChildObject("ExteriorEntities");
...
internal class FallingGFWreckTag : MonoBehaviour {
    ...
    private void doUnstableFX(bool sound, bool groan) {
        Collider[] boxes = this.GetComponentsInChildren<Collider>(true);
        for (int i = 0; i < 14; i++) {
            ...
            if (UnityEngine.Random.Range(0F, 1F) <= 0.75) {
                ...
                ParticleSystem fx = this.getSmokeFX();
                if (!fx)
                    continue;
        ...
        if (sound) {
            SoundManager.SoundData snd = groan
                ? groanSounds[UnityEngine.Random.Range(0, groanSounds.Length)]
                : hitSounds[UnityEngine.Random.Range(0, hitSounds.Length)];
            SoundManager.playSoundAt(
                snd,
                transform.position,
                false,
                120,
                groan ? 2 : UnityEngine.Random.Range(1F, 1.5F)
            );
            ...
            if (intensity > 0) {
                ...
                SNUtil.shakeCamera(
                    dur,
                    intensity * UnityEngine.Random.Range(1F, 2F),
    ...
    private ParticleSystem getSmokeFX() {
        ...
        GameObject child = go.getChildObject("xSmk");
    ...
    private void startFall() {
        ...
        foreach (StarshipDoor d in this.GetComponentsInChildren<StarshipDoor>()) {
