...
public class SanctuaryPlant : BasicCustomPlant, CustomHarvestBehavior {
    ...
    public SanctuaryPlant() : base(
        ...
        AddOnRegister(() => {
                SaveSystem.addSaveHandler(
                    ClassID,
                    new SaveSystem.ComponentFieldSaveHandler<SanctuaryPlantTag>().addField("lastHarvest")
                );
            }
        );
    }
    ...
    public GameObject tryHarvest(GameObject go) {
        return go.GetComponent<SanctuaryPlantTag>().tryHarvest()
            ? ObjectUtil.createWorldObject(seed.Info.TechType)
            : null;
    }
...
internal class SanctuaryPlantTag : MonoBehaviour, IHandTarget {
    ...
    public void OnHandHover(GUIHand hand) {
        if (this.isHarvested()) {
            ...
            HandReticle.main.SetText(HandReticle.TextType.Use, "SanctuaryPlantClick", true);
            HandReticle.main.SetTargetDistance(8);
        }
    }
