...
[Obsolete]
public class PostCoveTree : CustomPrefab {
    internal static readonly Dictionary<DecoPlants, float> templates = new() {
        { DecoPlants.LOST_BRANCHES_4, 0.33F },
        { DecoPlants.LOST_BRANCHES_5, 0.25F },
        { DecoPlants.LOST_BRANCHES_6, 0.08F },
    };

    internal static readonly Dictionary<string, PostCoveTree> types = new();
    ...
    public PostCoveTree(XMLLocale.LocaleEntry e, DecoPlants pfb) : base(e.key + "_" + pfb, e.name, e.desc) {
        ...
        AddOnRegister(() => { types[Info.ClassID] = this; });

        SetGameObject(GetGameObject);
    }

    public GameObject GetGameObject() {
        var go = ObjectUtil.createWorldObject(template.prefab);
    ...
    public static PDAScanner.EntryData postRegister() {
        var unlock = CraftingItems.getItem(CraftingItems.Items.ObsidianGlass).Info.TechType;
...
internal class PostCoveTreeTag : MonoBehaviour {
    ...
    private void Start() {
        ...
        foreach (var l in GetComponentsInChildren<PickPrefab>())
            l.gameObject.destroy(false);
        renderer = GetComponentInChildren<Renderer>();
        renderer.materials[1].color = Color.clear;
        var type = PostCoveTree.types[GetComponent<PrefabIdentifier>().ClassId];
        transform.localScale = Vector3.one * PostCoveTree.templates[type.template];
    }

    private void Update() {
        var
            hot = transform.position.y <
                  -1040; //VanillaBiomes.ILZ.isInBiome(transform.position) || WaterTemperatureSimulation.main.GetTemperature(transform.position) >= 90;
        var retexture = isHot != hot;
        isHot = hot;
        if (retexture) {
            var type = PostCoveTree.types[GetComponent<PrefabIdentifier>().ClassId];
            var n = type.template.getName();
            var tex = "Textures/Plants/PostCoveTree/" + (isHot ? "Hot" : "Cool") + n[n.Length - 1];
