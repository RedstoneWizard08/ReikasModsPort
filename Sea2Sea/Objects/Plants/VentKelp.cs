...
public class VentKelp : BasicCustomPlant, MultiTexturePrefab {
    ...
    internal static readonly Simplex3DGenerator noiseField =
        (Simplex3DGenerator)new Simplex3DGenerator(DateTime.Now.Ticks).setFrequency(0.1);

    internal static readonly SimplexNoiseGenerator heightNoiseField =
        (SimplexNoiseGenerator)new SimplexNoiseGenerator(873428712).setFrequency(1.0);
    ...
    internal static double getGrowthSpeed(float temp) {
        return temp <= idealTemperature
            ? MathUtil.linterpolate(temp, minTemperature, idealTemperature, 0, 1, true)
    ...
    public Dictionary<int, string> getTextureLayers(Renderer r) {
        return leavesOnlyRendering
            ? new Dictionary<int, string>() { { 0, "" }, { 1, "Leaves" } }
            : new Dictionary<int, string>() { { 0, "" }, { 1, "" } };
    }
    /*
...
internal class GlowKelpTag : MonoBehaviour {
    ...
    void Update() {
        ...
        if (time - lastAreaCheckTime >= 0.25) {
            ...
            } else if (transform.position.y >= -400 || MathUtil.getDistanceToLineSegment(
                           transform.position,
                           UnderwaterIslandsFloorBiome.wreckCtrPos1,
