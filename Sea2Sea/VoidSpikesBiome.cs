...
public class VoidSpikesBiome : CustomBiome { //FIXME: 3. disappearing spikes 4. custom levi
	...
	public override void register() {
		...
		this.createDiscoveryStoryGoal(10, SeaToSeaMod.miscLocale.getEntry("voidspikeenter"));
		...
		for (float i = -100; i <= length + 100; i += biomeVolumeRadius * 0.5F) {
			this.addAtmoFX(end500m + ((end900m - end500m).normalized * i));
		}
		this.addAtmoFX(end900m);
	...
	internal void tickTeleportCheck(SeaMoth sm) {
		...
		double dist = this.getDistanceToBiome(pos, false);
	...
	private bool isValidSpikeLocation(Vector3 vec) {
		return this.isInBiome(vec) && GenUtil.allowableGenBounds.Contains(vec + new Vector3(0, 0, -40)); //safety buffer
	}
	...
	public override bool isInBiome(Vector3 vec) {
		...
		return vec.y < -320 && this.getDistanceToBiome(vec, false) <= biomeVolumeRadius + 150;
	}

	public override double getDistanceToBiome(Vector3 vec) {
		return this.getDistanceToBiome(vec, true);
	}
	...
	public bool isPlayerInLeviathanZone(Vector3 pos) {
		return this.isInBiome(pos) && Vector3.Distance(pos, end900m) <= biomeVolumeRadius * 1.5F;
	}
