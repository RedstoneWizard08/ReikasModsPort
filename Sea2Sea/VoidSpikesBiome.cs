...
public class VoidSpikesBiome : CustomBiome { //FIXME: 3. disappearing spikes 4. custom levi
    ...
    public static readonly Vector3
        end500m = new(360, -550, 320); //new Vector3(925, -550, -2050);//new Vector3(895, -500, -1995);

    public static readonly Vector3
        end900m = new(800, -950, -120); //new Vector3(400, -950, -2275);//new Vector3(457, -900, -2261);
    ...
    public static readonly Vector3 waterColorFalloff = new(8, 4, 1);
    ...
    public static readonly VoidSpikesBiome instance = new();

    private readonly AtmoFX atmoFX = new();
    ...
    public override VanillaMusic[] getMusicOptions() {
        return [VanillaMusic.DUNES, VanillaMusic.BKELP, VanillaMusic.KOOSH, VanillaMusic.DEEPGRAND];
    }
    ...
    public override void register() {
        ...
        foreach (var gen in gens) {
            ...
            if (entryPoint == null ||
                Vector3.Distance(gen.position, end500m) < Vector3.Distance(entryPoint.position, end500m)) {
                entryPoint = gen;
            }
        }
        ...
        var e = SeaToSeaMod.SignalLocale.getEntry("voidpod");
        signal = SignalManager.createSignal(e);
        //signal.pdaEntry.addSubcategory("AuroraSurvivors");
        signal.addRadioTrigger(e.getString("sound"));
        ...
        GenUtil.registerWorldgen(
            PDAManager.getPage(PDA_KEY).getPDAClassID(),
            signalLocation + Vector3.down * 1.25F,
    ...
    private void addAtmoFX(Vector3 pos) {
        GenUtil.registerWorldgen(
            ...
            pos,
            Quaternion.identity,
            go => go.transform.localScale = Vector3.one * (100 + biomeVolumeRadius)
        );
    }

    internal void tickTeleportCheck(SeaMoth sm) {
        var pos = sm.transform.position;
    ...
    public double getDistanceToBiome(Vector3 vec, bool includeVoidSide) {
        var ret = MathUtil.getDistanceToLineSegment(vec, end500m, end900m); // endcap dist either < L
        if (includeVoidSide)
    ...
    private double getSpikeDepth(Vector3 vec) {
        var d1 = Vector3.Distance(end500m, vec) / length;
        //double d2 = Vector3.Distance(end900m, vec)/length;
        var interp = MathUtil.linterpolate(d1, 0, length, end500m.y, end900m.y);
        return MathUtil.getRandomPlusMinus((float)interp, 40F);
    }

    private Vector3 getSpikeLocation() {
        var init = MathUtil.interpolate(end500m, end900m, UnityEngine.Random.Range(0F, 1F));
    ...
    internal static WaveBob checkAndAddWaveBob(GameObject go, bool force) {
        ...
        var b = go.EnsureComponent<WaveBob>();
...
internal class AtmoFX : GenUtil.CustomPrefabImpl {
    [SetsRequiredMembers]
    ...
    public override void prepareGameObject(GameObject go, Renderer[] r) {
        var lw = go.EnsureComponent<LargeWorldEntity>();
        lw.cellLevel = LargeWorldEntity.CellLevel.Batch;
        var vol = go.EnsureComponent<AtmosphereVolume>();
        vol.affectsVisuals = true;
        vol.enabled = true;
        vol.fogMaxDistance = 100;
        vol.fogStartDistance = 20;
        vol.fogColor = new Color(
            vol.fogColor.r * 0.75F,
            vol.fogColor.g,
            Mathf.Min(1, vol.fogColor.b * 1.25F),
            vol.fogColor.a
        );
