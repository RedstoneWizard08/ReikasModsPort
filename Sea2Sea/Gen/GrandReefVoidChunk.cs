...
public sealed class GrandReefVoidChunk : WorldGenerator {
    ...
    public override bool generate(List<GameObject> generated) {
        ...
        for (int i = 0; i < nplants; i++) {
            ...
            Vector3? pos = this.getRandomPlantPosition();
            if (pos != null && pos.HasValue) {
                ...
                if (position.y - use.y < 1.9) {
                    ...
                    this.setPlantHeight(plantYAvg, p, go);
                    plants.Add(go);
                }
            }
        }
        ...
        for (int i = 0; i < 30; i++) {
            Vector3 pos = this.getRandomRockPosition(rockYAvg);
        ...
        for (int i = 0; i < 400 && genned < 5; i++) {
            Vector3 pos = this.getRandomMountPosition();
            pos.y = position.y - 5;
            while (position.y - pos.y < 9 && this.isColliding(pos, rocks)) {
                pos.y -= 0.1F;
            }

            if (!this.isColliding(pos, rocks)) {
                pos.y += 0.05F;
    ...
    private Vector3? getRandomPodPosition() {
        return this.getRandomFreePosition(pods, 3);
    }

    private Vector3? getRandomPlantPosition() {
        return this.getRandomFreePosition(plants, 1);
    }
    ...
    private Vector3? getRandomFreePosition(IEnumerable<GameObject> occupied, double minGap) {
        ...
        while ((rand == null || !rand.HasValue) && tries < 25) {
            rand = this.getRandomMountPosition();
