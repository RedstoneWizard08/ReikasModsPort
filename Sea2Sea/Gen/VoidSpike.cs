...
public sealed class VoidSpike : WorldGenerator {
    ...
    private static readonly string FLOATER_LIGHT = createPrefabCopy(
        "923a14c0-a7a2-49bd-a6fd-915d661582ee",
        LargeWorldEntity.CellLevel.VeryFar
    ).ClassID;
    ...
    public static void register() {
        addOre(
            new OreType(CustomMaterials.getItem(CustomMaterials.Materials.PRESSURE_CRYSTALS).ClassID, 825, 0.2),
            150
        );
    ...
    internal bool isValidPlantPos(Vector3 vec, string why) {
        return (!needsCenterSpace || (Vector3.Distance(vec._X0Z(), position._X0Z()) > 1.5 &&
                                      !why.ToLowerInvariant().Contains("membrain"))) &&
               (validPlantPosCheck == null || validPlantPosCheck(vec, why));
    }
    ...
    public override string ToString() {
        return base.ToString() + " , p=" + hasPod + " f=" + hasFloater + " flora=" + hasFlora + " R=" + oreRichness +
               " pr=" + plantRate;
    }
    ...
    public sealed class OreSpawner : CustomPrefab {
        [SetsRequiredMembers]
