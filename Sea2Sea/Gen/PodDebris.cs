...
public sealed class PodDebris : WorldGenerator {
        ...
        for (var i = 0; i < paperCount; i++) {
            li.Add(
                generateObjectInRange(
                    ...
                    4,
                    2,
                    papers[UnityEngine.Random.Range(0, papers.Count)].prefab.ClassID,
                    false
                )
            );
        }
    ...
    private GameObject generateObjectInRange(
        float dx,
        float dy,
        float dz,
        float offsetY = 0,
...
public sealed class PropPrefab : GenUtil.CustomPrefabImpl {
    ...
    public override void prepareGameObject(GameObject go, Renderer[] r) {
        go.removeComponent<MedicalCabinet>();
        go.removeComponent<Fabricator>();
