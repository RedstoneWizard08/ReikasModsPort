...
public sealed class VoidDebris : WorldGenerator {
    ...
    public void init() {
        databoxPrefab = GenUtil.getOrCreateDatabox(
            CraftingItems.getItem(CraftingItems.Items.HullPlating).Info.TechType,
            go => go.EnsureComponent<LargeWorldEntity>().cellLevel = LargeWorldEntity.CellLevel.Global
        );
    }
        ...
        for (int i = 0; i < 8; i++) {
            li.Add(
                this.generateObjectInRange(
                    ...
                    4,
                    -2,
                    PodDebris.papers[UnityEngine.Random.Range(0, PodDebris.papers.Count)]
                )
            );
        }
