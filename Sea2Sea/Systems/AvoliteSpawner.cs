...
public class AvoliteSpawner {
	...
	public void register() {
		...
		this.addItem(TechType.NutrientBlock, 10);
		this.addItem(TechType.DisinfectedWater, 8);
		this.addItem(TechType.Battery, 4);
		//addItem(TechType.Beacon, 3);

		this.addItem(TechType.PowerCell, 1);
		this.addItem(TechType.FireExtinguisher, 2);

		this.addItem(TechType.EnameledGlass, 5);
		this.addItem(TechType.Titanium, 25);
		this.addItem(TechType.ComputerChip, 3);
		this.addItem(TechType.WiringKit, 6);
		this.addItem(TechType.CopperWire, 20);

		this.addItem(TechType.Cap1, 2);
		this.addItem(TechType.Cap2, 3);
		this.addItem("dfabc84e-c4c5-45d9-8b01-ca0eaeeb8e65", 3);
		this.addItem(TechType.ArcadeGorgetoy, 2);
		this.addItem(TechType.PurpleVegetable, 3);

		this.addItem(CraftingItems.getItem(CraftingItems.Items.LathingDrone).ClassID, 1);
		this.addItem(CraftingItems.getItem(CraftingItems.Items.Motor).ClassID, 2);
	...
	public void postRegister() {
		ESHooks.scannabilityEvent += this.isItemMapRoomDetectable;
	}
	...
	private GameObject allocateItem(Transform t) {
		string pfb = this.tryFindItem(t.position);
		if (pfb == null)
			return null;
	...
	private string tryFindItem(Vector3 pos) {
		...
		if (UnityEngine.Random.Range(0, 2) == 0 && this.getCount(avo) < AVOLITE_COUNT && this.getClosest(avo, pos) >= 120 && WorldUtil.getObjectsNearWithComponent<AvoliteTag>(pos, 120).Count == 0) {
			return avo;
		}
		if (objectCountsToGo.Count == 0 || UnityEngine.Random.Range(0, 5) == 0) {
			return this.getCount("947f2823-c42a-45ef-94e4-52a9f1d3459c") < SCRAP_COUNT ? this.getRandomScrap().prefab : null;
		}
		else {
	...
	bool isValidPosition(Vector3 pos) {
		...
		return VanillaBiomes.MOUNTAINS.isInBiome(pos) && pos.x >= C2CHooks.gunCenter.x && Vector3.Distance(pos.setY(0), biomeCenter.setY(0)) <= 600 && Vector3.Distance(pos.setY(0), C2CHooks.gunCenter.setY(0)) >= 200 && Vector3.Distance(pos.setY(0), C2CHooks.mountainCenter.setY(0)) >= 360;
	}
