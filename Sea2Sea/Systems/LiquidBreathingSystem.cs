...
public class LiquidBreathingSystem {
    ...
    internal static readonly float ItemValue =
        SeaToSeaMod.ModConfig.getBoolean(C2CConfig.ConfigEntries.HARDMODE) ? 1800 : 2700;
    ...
    public bool IsO2BarAbleToFill(Player ep) {
        return !HasTankButNoMask() && !EnvironmentalDamageSystem.Instance.IsPlayerRecoveringFromPressure() &&
               (!HasLiquidBreathing() || IsInPoweredArea(ep) || !IsLiquidBreathingActive(ep));
    }
    ...
    public bool IsInPoweredArea(Player p) {
        ...
        if (p.IsUnderwater() || p.IsSwimming())
            return false;
        if (p.currentEscapePod && p.currentEscapePod == EscapePod.main && Story.StoryGoalManager.main &&
            EscapePod.main.fixPanelGoal != null &&
            Story.StoryGoalManager.main.IsGoalComplete(EscapePod.main.fixPanelGoal.key))
            return true;
        var v = p.GetVehicle();
        if (v && v.IsPowered())
            return true;
    ...
    public bool HasTankButNoMask() {
        return Inventory.main.equipment.GetTechTypeInSlot("Head") != C2CItems.rebreatherV2.TechType &&
               Inventory.main.equipment.GetTechTypeInSlot("Tank") == C2CItems.liquidTank.TechType;
    }
    ...
    public void UpdateOxygenGUI(uGUI_OxygenBar gui) {
        ...
        bar.texture = pink
            ? TextureManager.getTexture(SeaToSeaMod.ModDLL, "Textures/HUD/o2bar_liquid")
            : _baseO2BarTexture;
        bar.overlay = pink
            ? TextureManager.getTexture(SeaToSeaMod.ModDLL, "Textures/HUD/o2bar_liquid_bubble")
    ...
    public bool IsO2BarFlashingRed() {
        return Player.main.GetDepth() >= 400 && EnvironmentalDamageSystem.Instance.IsPlayerInOcean() &&
    ...
    internal void TickLiquidBreathing(bool has, bool active) {
        ...
        if (_meters) {
            ...
            if (has) {
                ...
                _meters.TreatmentBar.CurrentFillLevel = _hasTemporaryKharaaTreatment
                    ? 1
                    : KharaaTreatmentRemainingTime / GetTreatmentDuration();
            }
        }
        ...
        if (has && active) {
            ...
            if (_hasTemporaryKharaaTreatment && !_startedUsingTemporaryKharaaTreatment && !IsInPoweredArea(ep) &&
                HasLiquidBreathing()) {
    ...
    public class LiquidBreathingHUDMeterUnit : MonoBehaviour {
        ...
        private void Rebuild() {
            ...
            Foreground.sprite = Sprite.Create(
                TextureManager.getTexture(SeaToSeaMod.ModDLL, "Textures/LiquidBreathingFuelBar"),
        ...
        private void Update() {
            if (!FillBar) {
                ...
                Background.sprite = Sprite.Create(
                    TextureManager.getTexture(SeaToSeaMod.ModDLL, "Textures/LiquidBreathingFuelBarBack"),
                    new Rect(0, 0, 128, 128),
                    Vector2.zero
                );
            ...
            var f = CurrentFillLevel * 0.395F +
                    0.0625F; //this fraction is because it does not fill the bar, slightly more than 3/8 to use up texture
            if (overrideValue >= 0)
                f = overrideValue;
            Foreground.fillAmount = f;
            currentColor = CurrentFillLevel < 0.5F
                ? new Color(1, CurrentFillLevel * 2, 0, 1)
                : new Color(1 - (CurrentFillLevel - 0.5F) * 2, 1, 0, 1);
            if (CurrentFillLevel < 0.1) { //make flash if very low
                var ff = (1 + Mathf.Sin(
                    DayNightCycle.main.timePassedAsFloat * Mathf.Deg2Rad * (float)MathUtil.linterpolate(
                        CurrentFillLevel,
                        0.1,
                        0,
                        240,
