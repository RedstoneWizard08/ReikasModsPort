...
public class MoraleSystem {
    public static readonly MoraleSystem instance = new();
    ...
    private static readonly float
        MINUTE_DURATION_TO_PER_SECOND_CONVERSION = 0.6F; //x60 for time in seconds then /100 to per percent

    //percent per second
    private static readonly float CYCLOPS_MORALE_LOSS_SPEED =
        1F / (MINUTE_DURATION_TO_PER_SECOND_CONVERSION * CYCLOPS_MORALE_TIME_TO_DECAY);

    private static readonly float CYCLOPS_MORALE_GAIN_SPEED =
        1F / (MINUTE_DURATION_TO_PER_SECOND_CONVERSION * CYCLOPS_MORALE_TIME_TO_FILL);

    private static readonly float BASE_MORALE_LOSS_SPEED =
        1F / (MINUTE_DURATION_TO_PER_SECOND_CONVERSION * BASE_MORALE_TIME_TO_DECAY);

    private static readonly float BASE_MORALE_GAIN_SPEED =
        1F / (MINUTE_DURATION_TO_PER_SECOND_CONVERSION * BASE_MORALE_TIME_TO_FILL);
    ...
    public static uint printMoraleForDebug;

    private static readonly MoraleInfluence NO_MORALE_EFFECT = new(
        new ConstantValue<Player>(0),
        new ConstantValue<Player>(0)
    );

    private readonly Dictionary<BiomeBase, AmbientMoraleInfluence> biomeEffect = new();
    private readonly Dictionary<string, float> goalMorale = new();
    private readonly List<ConditionalMoraleInfluence> persistentEffects = [];
    ...
    public void register() {
        ...
        registerBiomeEffect(VanillaBiomes.Ilz).setEnv(-0.4F, 0, EnvContext.FREEDIVE)
            .setEnv(-0.1F, 0, EnvContext.SEAMOTH);
        registerBiomeEffect(VanillaBiomes.Bloodkelp).setEnv(-0.1F, -20F, EnvContext.FREEDIVE)
            .setEnv(0, -10F, EnvContext.SEAMOTH);
        registerBiomeEffect(VanillaBiomes.Bloodkelpnorth).setEnv(-0.125F, -25F, EnvContext.FREEDIVE)
            .setEnv(0, -12F, EnvContext.SEAMOTH);
        ...
        registerBiomeEffect(VanillaBiomes.Grandreef).setEnv(0.25F, 5F).setEnv(0, 2.5F, EnvContext.FREEDIVE)
            .setEnv(0, 10F, EnvContext.SEABASE);
        registerBiomeEffect(VanillaBiomes.Jellyshroom).setEnv(-5, -100, EnvContext.CYCLOPS)
            .setEnv(0, 10F, EnvContext.SEABASE);
        registerBiomeEffect(VanillaBiomes.Shallows).setEnv(0.05F, 0).setEnv(-1, -100, EnvContext.CYCLOPS)
            .setEnv(0.125F, 5F, EnvContext.SEABASE);
        ...
        registerPersistentEffect(ep => ep.GetInfectionAmount() > 0.9F, -0.2F, -5);

        registerLifepodMoraleShifts(StoryGoals.getRadioPlayGoal(StoryGoals.POD12RADIO), StoryGoals.POD12);
        registerLifepodMoraleShifts(StoryGoals.getRadioPlayGoal(StoryGoals.POD6RADIO), StoryGoals.POD6B);
        registerLifepodMoraleShifts(StoryGoals.getRadioPlayGoal(StoryGoals.POD17RADIO), StoryGoals.POD17); //ozzy
        registerLifepodMoraleShifts(
            StoryGoals.getRadioPlayGoal(StoryGoals.POD3RADIO),
            StoryGoals.POD3,
            2F
        ); //x2 strength since usually first
        registerLifepodMoraleShifts(StoryGoals.getRadioPlayGoal(StoryGoals.POD2RADIO), StoryGoals.POD2);
        registerLifepodMoraleShifts(
            StoryGoals.getRadioPlayGoal(StoryGoals.POD13RADIO),
            StoryGoals.POD13,
            0.5F
        ); //khasar; half because Alterra vs Mongolians
        registerLifepodMoraleShifts(StoryGoals.getRadioPlayGoal(StoryGoals.POD7RADIO), StoryGoals.POD7);
        registerLifepodMoraleShifts(
            StoryGoals.getRadioPlayGoal(StoryGoals.POD19RADIO),
            StoryGoals.ISLAND_RENDEZVOUS,
            1.5F
        ); //only clear once rendezvous fails
        registerLifepodMoraleShifts(
            StoryGoals.getRadioPlayGoal(StoryGoals.POD4RADIO),
            StoryGoals.POD4
        ); //also add strong negative impulse because of realization of danger
        goalMorale[StoryGoals.POD4] = -25;

        registerPersistentEffect(
            StoryGoals.POD19RENDEZVOUS,
            false,
            100,
            sunbeamCrisisDuration * 2,
            StoryGoals.ISLAND_RENDEZVOUS,
            -50,
            sunbeamCrisisDuration / 2F
        ); //survivors on an island...except not

        registerLifepodMoraleShifts("rescuepdalog", "treepda", 1.5F); //hint of rescue...never mind
        registerLifepodMoraleShifts(StoryGoals.getRadioPlayGoal(SeaToSeaMod.TreaderSignal.storyGate), "treaderpod");
        registerLifepodMoraleShifts(StoryGoals.getRadioPlayGoal(VoidSpikesBiome.instance.getSignalKey()), "voidpod");
        registerLifepodMoraleShifts(StoryGoals.getRadioPlayGoal(SeaToSeaMod.CrashMesaRadio.key), "crashmesa");

        registerPermanentGoalEffect(
            StoryGoals.getRadioPlayGoal(StoryGoals.ALTERRA_HQ),
            0,
            5
        ...
        StoryHandler.instance.addListener(onStoryGoal);
    ...
    public void addItemMorale(CustomPrefab pfb, float amt) {
        addItemMorale(pfb.TechType, amt);
    }

    public void addItemMorale(TechType tt, float amt) {
        FirstObtainmentSystem.instance.registerEvent(tt, () => shiftMorale(amt));
    }
    ...
    public AmbientMoraleInfluence registerBiomeEffect(BiomeBase bb) {
        var amb = new AmbientMoraleInfluence();
    ...
    public void registerLifepodMoraleShifts(string radioGoal, string pdaGoal, float strength = 1) {
        registerPersistentEffect(
            radioGoal,
            false,
            GENERIC_RADIO_MORALE_BOOST * strength,
            GENERIC_RADIO_MORALE_DURATION,
            pdaGoal,
            GENERIC_DEAD_LIFEPOD_MORALE_IMPACT * strength,
            GENERIC_DEAD_LIFEPOD_MORALE_DURATION
        );
    }

    public void registerBaselineAdjustment(ProgressionTrigger check, float force, float baseline) {
        registerPersistentEffect(ep => check.isReady(ep), force, baseline);
    }

    public void registerBaselineAdjustment(ProgressionTrigger check, float effect, bool isForce) {
        registerPersistentEffect(ep => check.isReady(ep) ? effect : 0, isForce);
    }

    public void registerPersistentEffect(
        string goal1,
        bool isForce,
        float effect1Initial,
        float fadeTime1,
        string goal2,
        float effect2Initial,
        float fadeTime2 = -1
    ) {
        registerPersistentEffect(
            ep => {
                if (StoryGoalManager.main.IsGoalComplete(goal1)) {
                    if (StoryGoalManager.main.IsGoalComplete(goal2)) {
                        ...
                        var since = StoryHandler.instance.getTimeSince(goal2);
                        return since >= fadeTime2
                            ? 0
                            : (float)MathUtil.linterpolate(since, 0, fadeTime2, effect2Initial, 0, true);
                    } else {
                        var since = StoryHandler.instance.getTimeSince(goal1);
                        return since >= fadeTime1
                            ? 0
    ...
    public void registerPermanentGoalEffect(string goal, float force, float baseline) {
        registerPersistentEffect(ep => StoryGoalManager.main.IsGoalComplete(goal), force, baseline);
    }
    ...
    public void registerPersistentEffect(Func<Player, float> effect, bool isForce) { //legacy call
        ...
        registerPersistentEffect(
            new ConditionalMoraleInfluence(new MoraleInfluence(new CallbackValue<Player>(effect), isForce), p)
        );
    }
    ...
    public void registerPersistentEffect(
        Predicate<Player> check,
        DynamicValue<Player> force,
        DynamicValue<Player> baseline
    ) {
        registerPersistentEffect(check, new MoraleInfluence(force, baseline));
    }
    ...
    private void onStoryGoal(string goal) {
        ...
        switch (goal) {
            ...
            case "OnPlayRadioBounceBack": {
                var e = Player.main.gameObject.EnsureComponent<MoraleEffect9999>();
            ...
            case StoryGoals.SUNBEAM_DESTROY_FAR: {
                instance.shiftMorale(200);
                var witness = goal == StoryGoals.SUNBEAM_DESTROY_NEAR;
                var e = Player.main.gameObject.EnsureComponent<MoraleDelaySunbeamDestroy>();
                e.onsite = witness;
    ...
    private class MoraleEffect9999 : MonoBehaviour {
        private int times;

        private void firstFire() {
            instance.shiftMorale(20);
        }

        private void trigger() {
            times++;
            instance.shiftMorale(-6 * times);
            if (times >= 5) {
    ...
    private class MoraleDelaySunbeamDestroy : MonoBehaviour {
        internal bool onsite;
        private bool rampedAnticipate;
        private bool rampedAnticipate2;

        private void triggerAnticipate() {
            var amt = rampedAnticipate2 ? -0.5F :
                rampedAnticipate ? -0.25F : -0.075F; //at 20/s, this is therefore 1.5/s and 5/s
            instance.shiftMorale(onsite ? amt : amt * 0.67F);
        }

        private void triggerAnticipateRamp() {
            rampedAnticipate = true;
        }

        private void triggerAnticipateRamp2() {
        ...
        private void trigger() {
            ...
            instance.shiftMorale(-200);
            CancelInvoke(nameof(triggerAnticipate));
            Destroy(this, 0.5F);
        }
    }
...
*/
    ...
    public void tick(Player ep) {
        if (!ep || !ep.liveMixin || GameModeUtils.currentEffectiveMode == GameModeOption.Creative ||
            GameModeUtils.currentEffectiveMode == GameModeOption.Freedom ||
            !DIHooks.IsWorldLoaded() || /*ep.cinematicModeActive*/uGUI.isIntro ||
            EscapePod.main.IsPlayingIntroCinematic() || LaunchRocket.launchStarted ||
            StoryGoalManager.main.IsGoalComplete(StoryGoals.LAUNCH_ROCKET) || !MainCameraControl.main.enabled) {
        ...
        if (!barsRoot) {
            ...
            if (o2bar) {
                ...
                if (!bar) {
                    var wb = barsRoot.getChildObject("WaterBar");
                    var fb = barsRoot.getChildObject("FoodBar");
                    var mb = wb.clone().setName("MoraleBar");
                    mb.transform.SetParent(wb.transform.parent);
                    var diff = fb.transform.localPosition - wb.transform.localPosition;
                    ...
                    var refBar = mb.GetComponentInChildren<uGUI_WaterBar>();
                ...
                var bcg = barsRoot.getChildObject("BackgroundQuad/Quad").GetComponent<Image>();
                var tex = TextureManager.getTexture(AqueousEngineeringMod.modDLL, "Textures/HUDBarsBCG");
        ...
        var v = ep.GetVehicle();
        var bb = BiomeBase.GetBiome(ep.transform.position);
        ...
        if (ep.currentSub) {
            ...
            if (time - lastDecoCheckTime > 0.5F) {
                ...
                currentRoom = BaseRoomSpecializationSystem.instance.getPlayerRoomType(
                    ep,
                    out currentDecoLevel,
                    out var thresh
                );
            }

            var cyclops = ep.currentSub.isCyclops;
            env = cyclops ? EnvContext.CYCLOPS : EnvContext.SEABASE;
            var min = cyclops ? CYCLOPS_MIN_DECO : BASE_MIN_DECO;

            if (currentDecoLevel < min) {
                var lossLim = cyclops ? CYCLOPS_MORALE_LOSS_SPEED : BASE_MORALE_LOSS_SPEED;
                delta = -(float)MathUtil.linterpolate(currentDecoLevel, 0, min, lossLim, 0, true);
            } else {
                var gainCeil = cyclops ? CYCLOPS_MAX_DECO : BASE_MAX_DECO;
                var gainLim = cyclops ? CYCLOPS_MORALE_GAIN_SPEED : BASE_MORALE_GAIN_SPEED;
            ...
            if (v is SeaMoth sm) {
                ...
                resetPrawnTime();
                delta += (float)MathUtil.linterpolate(
                    sm.useRigidbody.velocity.magnitude,
                    5,
                    20,
                    0,
                    0.75
                ); //up to 0.75% per second if moving fast
                if (amb != null)
                ...
                var bonus = 0.5F *
                            (1 - timeInPrawn /
                                120F); //"sense of limitless power" -> 0.5%/s fading over 120 seconds (total morale maximum: +30)
                if (bonus > 0)
                    delta += bonus;
                if (amb != null)
        ...
        if (time - lastBaselineCheckTime > 1F) {
            ...
            foreach (var amt in persistentEffects) {
                if (!amt.isActive(ep))
                    continue;
    ...
    private class MoraleBar : uGUI_WaterBar {
        ...
        private void Start() {
            ...
            var tex = TextureManager.getTexture(AqueousEngineeringMod.modDLL, "Textures/MoraleIcon");
            iconImage.sprite = TextureManager.createSprite(tex, 100);
            var rt = iconImage.gameObject.GetComponent<RectTransform>();
        ...
        private void updateValue() {
            var tgt = targetPercentage / 100F;
            curr = Mathf.SmoothDamp(curr, tgt, ref vel, dampSpeed);
            bar.value = curr;
            var num = Mathf.CeilToInt(targetPercentage);
            if (cachedValue != num) {
                cachedValue = num;
        ...
        internal void setValue(float pct) {
            var time = DayNightCycle.main.timePassedAsFloat;
            var diff = Mathf.Abs(pct - targetPercentage);
            if (diff <= 0.5F && time - lastSetTime <= 1)
                return;
            targetPercentage = pct;
            lastSetTime = time;
            if (diff > 10) {
                var maxScale = 1f + Mathf.Clamp01((diff - 10F) / 50F);
                Punch(2.5f, maxScale);
            }
        }

        private new void LateUpdate() {
            var flag = showNumbers;
            showNumbers = false;
            var main = Player.main;
            if (main != null) {
                var num = Mathf.Clamp01(curr / pulseReferenceCapacity);
                var time = 1f - num;
                ...
                var time2 = Time.time;
                var num3 = 0.02f;
                var num4 = time2 - lastFixedUpdateTime;
                ...
                for (var i = 0; i < num5; i++) {
                    var num6 = rotationCurrent;
                    var num7 = showNumbers ? 180f : 0f;
                    MathExtensions.Spring(
                        ref rotationVelocity,
                        ref rotationCurrent,
                        num7,
                        rotationSpringCoef,
                        num3,
                        rotationVelocityDamp,
                        rotationVelocityMax
                    );
    ...
    public class MoraleInfluence {
        ...
        public MoraleInfluence(DynamicValue<Player> val, bool isForce) : this(
            isForce ? val : new ConstantValue<Player>(0),
            isForce ? new ConstantValue<Player>(0) : val
        ) {
        }

        public override string ToString() {
            return $"F=({moralePerSecondForce.ToString("0.00")}) B=({moraleBaselineShift.ToString("0.00")})";
        }
    }
    ...
    public class AmbientMoraleInfluence {
        ...
        public AmbientMoraleInfluence(MoraleInfluence m, MoraleInfluence c, MoraleInfluence v = null) :
            this(m, c, v, v) {
        }
        ...
        public AmbientMoraleInfluence setEnv(float force, float baseline, EnvContext e) {
            var inf = new MoraleInfluence(force, baseline);
            switch (e) {
                case EnvContext.FREEDIVE:
                    freeDiving = inf;
                    break;
    ...
    private class SaveHook : SaveSystem.PlayerSaveHook {
