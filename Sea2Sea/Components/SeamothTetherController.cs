    ...
    void FixedUpdate() {
        ...
        if (!tetherRoot) {
            ...
            if (!tetherRoot) {
                ...
                PropulsionCannon template = ObjectUtil.lookupPrefab(TechType.PropulsionCannon).GetResult()
...
class Tether {
    ...
    internal void pull(SeamothTetherController mgr, float dT) {
        ...
        float f = animal ? 2 : 1;
        Vector3 tgt = mgr.transform.position +
        ...
        Vector3 vector = target.velocity + (Vector3.Normalize(distance) * (animal ? 300 : 1200) * d * dT /
