            ...
            (v, m, ii, li) => {
                if (m) {
                    ...
                    go.transform.localScale =
                        Vector3.one * (v.transform.position - Player.main.transform.position).magnitude;
                    Player.main.gameObject.EnsureComponent<DelayedKill>().initialize(
                        v == Player.main.GetVehicle() ? 0.05F : 0.5F,
