...
public class BaseRoomSpecializationSystem { //TODO 2.0 handle large rooms
    ...
    private static readonly HashSet<string> lockers = [];
    private static readonly Dictionary<string, float> decoRatings = new();
    private static readonly Dictionary<TechType, float> itemDecoRatings = new();
    private static readonly Dictionary<string, RoomTypes[]> objectTypeMappings = new();

    public static bool debugRoomCompute = false;

    public static readonly BaseRoomSpecializationSystem instance = new();
    ...
    private BaseRoomSpecializationSystem() {
        ...
        objectTypeMappings[ACU_PREFAB] = [RoomTypes.ACU];
        objectTypeMappings["87f5d3e6-e00b-4cf3-be39-0a9c7e951b84"] = [RoomTypes.AGRICULTURAL]; //indoor growbed			
        objectTypeMappings["769f9f44-30f6-46ed-aaf6-fbba358e1676"] = [RoomTypes.POWER]; //bioreactor
        objectTypeMappings["864f7780-a4c3-4bf2-b9c7-f4296388b70f"] = [RoomTypes.POWER]; //nuclear reactor
        objectTypeMappings["51eba507-317c-46bf-adde-4459dc8e002e"] = [RoomTypes.LEISURE]; //vending machine
        objectTypeMappings["b09a156d-d3cf-455a-848d-a9a8cad2b811"] = [RoomTypes.LEISURE]; //coffee
        objectTypeMappings["5d63470b-2705-4963-be4b-2e11e8dc6f2e"] = [RoomTypes.LEISURE]; //bench
        objectTypeMappings["4cb154ef-bdb6-4ff4-9107-f378ce21a9b7"] = [RoomTypes.LEISURE]; //bench
        objectTypeMappings["26cdb865-efbd-403c-8873-92453bcfc935"] = [RoomTypes.LEISURE]; //chair
        objectTypeMappings["4f0e304b-0d25-4f9c-a6ba-1b1a6bf029b0"] = [RoomTypes.LEISURE, RoomTypes.WORK]; //chair
        objectTypeMappings["cbeca4bd-cba4-4905-89fd-2470aaa204b1"] = [RoomTypes.LEISURE, RoomTypes.WORK]; //chair
        objectTypeMappings["cf522a95-3038-4759-a53c-8dad1242c8ed"] = [RoomTypes.LEISURE, RoomTypes.WORK]; //desk
        objectTypeMappings["7370e7a0-ebc0-4c33-9997-7084c11a55b0"] = [RoomTypes.LEISURE, RoomTypes.WORK]; //counter
        objectTypeMappings["cdb374fd-4f38-4bef-86a3-100cc87155b6"] = [RoomTypes.LEISURE]; //bed
        objectTypeMappings["c3994649-d0da-4f8c-bb77-1590f50838b9"] = [RoomTypes.LEISURE]; //bed
        objectTypeMappings["dffaf40f-9fbc-4553-9b35-3f939c76c283"] = [RoomTypes.LEISURE]; //bed
        objectTypeMappings["2f2d8419-c55b-49ac-9698-ecb431fffed2"] = [RoomTypes.MECHANICAL]; //water filter			
        objectTypeMappings["cdade216-3d4d-4adf-901c-3a91fb3b88c4"] = [RoomTypes.WORK]; //centrifuge		
        objectTypeMappings["bef7bc0b-149d-4342-bbb4-329047685578"] = [RoomTypes.WORK]; //fragment analyzer
        objectTypeMappings["c0175cf7-0b6a-4a1d-938f-dad0dbb6fa06"] = [
            RoomTypes.WORK, RoomTypes.MECHANICAL, RoomTypes.LEISURE,
        ]; //medical cabinet
        objectTypeMappings["c9bdcc4d-a8c6-43c0-8f7a-f86841cd4493"] = [RoomTypes.WORK]; //specimen analyzer
        objectTypeMappings["5c06baec-0539-4f26-817d-78443548cc52"] = [
            RoomTypes.LEISURE, RoomTypes.WORK, RoomTypes.MECHANICAL,
        ]; //radio	
        //no need to map chargers; that is handled directly via instanceof Charger in the computation code

        //FCS
        objectTypeMappings["CabinetTVStand"] = [RoomTypes.LEISURE];
        objectTypeMappings["FCSToilet"] = [RoomTypes.LEISURE];
        objectTypeMappings["fcsmicrowave"] = [RoomTypes.LEISURE];
        objectTypeMappings["FCSCrewBunkBed"] = [RoomTypes.LEISURE];
        objectTypeMappings["UniversalCharger"] = [RoomTypes.MECHANICAL];
        objectTypeMappings["MiniMedBay"] = [RoomTypes.LEISURE];
        objectTypeMappings["MiniFountainFilter"] = [RoomTypes.LEISURE];
        objectTypeMappings["Seabreeze"] = [RoomTypes.LEISURE, RoomTypes.STORAGE];
        objectTypeMappings["EnergyPillVendingMachine"] = [RoomTypes.LEISURE, RoomTypes.MECHANICAL];
        ...
        objectTypeMappings["fcsCuringCabinet"] = [RoomTypes.WORK];
        objectTypeMappings["AlterraGen"] = [RoomTypes.POWER];
        objectTypeMappings["AlterraStorage"] = [RoomTypes.STORAGE];
        objectTypeMappings["HydroponicHarvester"] = [RoomTypes.AGRICULTURAL];
        objectTypeMappings["Replicator"] = [RoomTypes.MECHANICAL];
        objectTypeMappings["MatterAnalyzer"] = [RoomTypes.WORK];
        objectTypeMappings["AutoCrafter"] = [RoomTypes.MECHANICAL];
        objectTypeMappings["PeeperLoungeBar"] = [RoomTypes.LEISURE];
        objectTypeMappings["FireExtinguisherRefueler"] = [RoomTypes.MECHANICAL];
        objectTypeMappings["DSSFloorServerRack"] = [RoomTypes.MECHANICAL];
        ...
        objectTypeMappings["FCSShower"] = [RoomTypes.LEISURE];
        objectTypeMappings["FCSSink"] = [RoomTypes.LEISURE];
        objectTypeMappings["FCSJukebox"] = [RoomTypes.LEISURE];
        objectTypeMappings["FCSJukeboxSpeaker"] = [RoomTypes.LEISURE];
        objectTypeMappings["FCSJukeBoxSubWoofer"] = [RoomTypes.LEISURE];
        objectTypeMappings["FCSStove"] = [RoomTypes.LEISURE, RoomTypes.WORK];
        objectTypeMappings["tableSmartTV"] = [RoomTypes.LEISURE];
        objectTypeMappings["mountSmartTV"] = [RoomTypes.LEISURE];
        objectTypeMappings["Sofa1"] = [RoomTypes.LEISURE];
        objectTypeMappings["Sofa2"] = [RoomTypes.LEISURE];
        objectTypeMappings["Sofa3"] = [RoomTypes.LEISURE];
        objectTypeMappings["NeonBarStool"] = [RoomTypes.LEISURE];
        objectTypeMappings["FCSCrewBunkBed"] = [RoomTypes.LEISURE];
        objectTypeMappings["PatreonStatue"] = [RoomTypes.LEISURE];
        //
    ...
    public float getLeisureDecoThreshold(BaseCell cell) {
        return getLeisureDecoThreshold(isLargeRoom(cell));
    }
    ...
    public void registerModdedObject(CustomPrefab pfb, float deco, params RoomTypes[] types) {
        if (pfb == null)
            return;
        this.registerModdedObject(pfb.ClassID, deco, types);
        setDisplayValue(pfb.TechType, deco);
        if (pfb is BasicCustomPlant plant) {
            var seed = plant.seed;
            if (seed != null) {
    ...
    internal RoomTypes getType(
        ...
        out float decoRating,
        bool debug = false
    ) {
        debug |= debugRoomCompute;
        var options = new HashSet<RoomTypes>((IEnumerable<RoomTypes>)Enum.GetValues(typeof(RoomTypes)));
        //if (bc.GetComponentInChildren<BaseNuclearReactor>() || bc.GetComponentInChildren<BaseBioReactor>())
        //	options.Add(RoomTypes.POWER);
        var lockerCount = 0;
        var agriCount = 0;
        var deco = new DecoRatingTracker();
        var large = isLargeRoom(bc);
        foreach (var pi in li) {
            var cc = pi.GetComponent<Constructable>();
            if (cc && !cc.constructed) {
                deco.addDeco(-2F);
                continue;
            }

            var obj = getObjectType(pi);
            //if (obj.Length == 1)
            //	options.AddRange(obj);
            if (obj.Length > 1 ||
                (obj.Length == 1 &&
                 obj[0] != RoomTypes
                     .UNSPECIALIZED)) //do not rule out all other room types just because of an unspecialized item
                options.IntersectWith(obj);
            if (lockers.Contains(pi.ClassId))
                lockerCount++;
            if (Array.IndexOf(obj, RoomTypes.AGRICULTURAL) >= 0)
                agriCount++;
            getDecoRating(pi, deco, large, debug);
            //SNUtil.writeToChat("Cell "+bc.transform.position+": Object "+pi.name+" > "+getObjectType(pi).toDebugString()+" #"+getDecoRating(pi));
        }

        var hasGlassRoof = bc.gameObject.getChildObject("BaseRoomInteriorTopGlass") != null;
        var plantPanels = bc.gameObject.getChildObjects("BaseRoomPlanterSide").Count;
        var windows = bc.gameObject.getChildObjects("BaseRoomWindowSide").Count;
        var sideWindows = windows > 0;
        if (hasGlassRoof)
        ...
        if (windows > 0) {
            deco.addDeco(
                windows * getWindowDecoValue(bb, bc, hasGlassRoof, debug)
            ); //windows, rating is base location dependent
            if (sideWindows) {
                HashSet<int> seen = [];
                WorldUtil.getObjectsNear<GameObject>(
                    bc.transform.position,
                    25,
                    go => {
                        if (go.activeSelf && !seen.Contains(go.GetInstanceID()) &&
                            Mathf.Abs(go.transform.position.y - bc.transform.position.y) <= 10) {
                            seen.Add(go.GetInstanceID());
                            var p = go.GetComponent<Planter>();
                            if (p && !p.isIndoor) {
                                getInventoryDecoValue(p.GetComponent<StorageContainer>(), deco, debug /*, 0.8F*/);
                                //if (debug)
        ...
        var lockerThresh = large ? 8 : 5;
        if (lockerCount >= lockerThresh) { //do before leisure/agri are removed
            options.Add(RoomTypes.STORAGE);
            if (options.Count == Enum.GetValues(typeof(RoomTypes)).Length) {
                //if only lockers + other generics, will not have any filtering
                options.Clear();
                options.Add(RoomTypes.STORAGE);
            }
        }

        if (decoRating < getLeisureDecoThreshold(bc))
    ...
    private float getWindowDecoValue(BaseRoot bb, BaseCell bc, bool hasGlassRoof, bool debug) {
        ...
        if (pos.y >= -1)
            return 0.1F;
        var depth = -pos.y;
        var b = BiomeBase.GetBiome(pos);
        var scenery = Mathf.Clamp(b.SceneryValue, -1, 2.5F);
        ...
        if ((b == VanillaBiomes.Redgrass || b == VanillaBiomes.Mushroom) && depth < 50) {
            ...
            scenery = 0;
        } else if ((b == VanillaBiomes.Crag || b == VanillaBiomes.Sparse || b == VanillaBiomes.Grandreef) &&
                   depth < 150) {
            scenery = 0;
            ...
            var biome = WaterBiomeManager.main.GetBiome(pos, false).ToLowerInvariant();
        ...
        if (scenery > 0) {
            ...
            WorldUtil.getObjectsNear<GameObject>(
                ...
                100,
                go => {
                    if (go.activeSelf && go.transform.position.y >= pos.y - 50 &&
                        (hasGlassRoof || go.transform.position.y <= pos.y + 50) &&
                        isExternalPropObject(bb, go.transform)) {
                        totalFound++;
            ...
            if (totalFound <=
                50) //basically nothing found, must be open water (will never be zero beacuse of ocean, occasional fish, etc)
                scenery = 0;
            else if
                (objectsFound <=
    ...
    private void getDecoRating(PrefabIdentifier pi, DecoRatingTracker deco, bool largeRoom, bool debug) {
        var pf = pi.GetComponent<PictureFrame>();
        if (pf) {
            deco.addObject(pi, pf.current == PictureFrame.State.None ? -1 : 3, debug, 9999);
            return;
        }

        var sg = pi.GetComponent<Sign>();
        if (sg) {
            var text = sg.GetComponentInChildren<uGUI_SignInput>().text;
            var amt = string.IsNullOrEmpty(text) || text.Equals("sign", StringComparison.InvariantCultureIgnoreCase)
                ? 0
                : 0.25F;
            if (amt > 0) {
        ...
        // }
        var disp = pi.GetComponent<ItemDisplayLogic>();
        if (disp) {
            var tt = disp.displayType;
            var pp = disp.currentItem;
            float amt = 0;
            if (tt == TechType.None || !pp)
                amt = -0.25F;
            else
                amt = getItemDecoValue(disp.displayType);
            if (amt > 0)
        ...
        var p = pi.GetComponent<Planter>();
        if (p) {
            deco.addObject(pi, getDecoRating(pi.ClassId), debug);
            getInventoryDecoValue(p.GetComponent<StorageContainer>(), deco, debug, repLimit: 3);
            return;
        }

        var a = pi.GetComponent<Aquarium>();
        if (a) {
            deco.addObject(pi, 2, debug); //even empty has some value
            getInventoryDecoValue(a.GetComponent<StorageContainer>(), deco, debug, repLimit: 3);
            return;
        }

        var wp = pi.GetComponent<AcuCallbackSystem.AcuCallback>();
        if (wp) {
            getACUDecoValue(wp, largeRoom, deco, debug);
            return;
        }
    ...
    private void getInventoryDecoValue(
        StorageContainer sc,
        DecoRatingTracker deco,
        bool debug,
        float factor = 1,
        int repLimit = 2
    ) {
    ...
    private void getInventoryDecoValue(
        ...
        foreach (var pp in li) {
            deco.addObject(pp.GetTechType(), pp.gameObject, getItemDecoValue(pp) * factor, debug, repLimit);
        }
    }
    ...
    internal RoomTypes[] getObjectType(GameObject go, string id) {
        return go.GetComponent<Charger>() ? [RoomTypes.MECHANICAL] :
            go.GetComponent<Crafter>() ? [RoomTypes.WORK] :
            objectTypeMappings.ContainsKey(id) ? objectTypeMappings[id] : [RoomTypes.UNSPECIALIZED];
    }

    internal RoomTypes getSavedType(Component go) {
        return getSavedType(go, out var deco, out var decoThresh);
    }

    internal RoomTypes getSavedType(Component go, out float deco, out float decoThresh) {
        var rt = go.gameObject
            .FindAncestor<RoomTypeTracker>(); //will find the one on the main GO, else BaseCell if possible
        if (!rt) {
            var bb = go.GetComponentInParent<BaseRoot>();
            if (bb) {
                var bc = ObjectUtil.getBaseRoom(bb, go.gameObject);
                if (bc)
                    rt = bc.GetComponent<RoomTypeTracker>();
            }
        }

        decoThresh = rt ? getLeisureDecoThreshold(rt.getRoom()) : 9999;
        deco = rt ? rt.getDecorationValue() : 0;
        return rt ? rt.getType() : RoomTypes.UNSPECIALIZED;
    }

    public RoomTypes getPlayerRoomType(Player ep) {
        return getPlayerRoomType(ep, out var deco, out var decoThresh);
    }
    ...
    public RoomTypes getPlayerRoomType(Player ep, out float deco, out float decoThresh) {
        ...
        var rt = bc.GetComponent<RoomTypeTracker>();
        deco = rt ? rt.getDecorationValue() : 0;
        return rt ? rt.getType() : RoomTypes.UNSPECIALIZED;
    }
    ...
    public bool storageHasDecoValue(StorageContainer sc) {
        ...
        return (pi && pi.ClassId == AqueousEngineeringMod.displayBlock.ClassID) ||
               (pi && pi.ClassId == STANDING_LOCKER_PREFAB && hasLockers) || sc.GetComponent<Aquarium>() ||
               sc.GetComponent<Planter>();
    }
    ...
    public void recomputeBaseRoom(BaseRoot bb, BaseCell cell, bool notify) {
        var tr = cell.GetComponent<RoomTypeTracker>();
        if (tr && tr.justComputed)
            return;
        var prev = tr ? tr.getType() : RoomTypes.UNSPECIALIZED;
        var prevD = tr ? tr.getDecorationValue() : 0;
        var li = ObjectUtil.getBaseObjectsInRoom(bb, cell);
        //SNUtil.writeToChat("Checking room type for "+go);
        var type = getType(bb, cell, li, out var deco, debugRoomCompute);
        //SNUtil.writeToChat("Room at "+cell.transform.position+" is now type "+type+"; decoration rating = "+deco.ToString("0.00"));
        var name = AqueousEngineeringMod.roomLocale.getEntry(Enum.GetName(typeof(RoomTypes), type)).desc;
        if (debugRoomCompute || (notify && AqueousEngineeringMod.config.getBoolean(AEConfig.ConfigEntries.ROOMCHAT) &&
                                 (prev != type || !Mathf.Approximately(prevD, deco)))) {
    ...
    public void recomputeBaseRooms(BaseRoot root, float delay = 0) {
        if (delay > 0) {
            var q = root.gameObject.AddComponent<BaseUpdateQueue>();
            q.delay = delay;
            ...
            foreach (var cell in bc) {
                recomputeBaseRoom(root, cell, debugRoomCompute);
            }
        }
    }
    ...
    private class BaseUpdateQueue : MonoBehaviour {
        internal float delay;

        private void recompute() {
            if (!DIHooks.IsWorldLoaded()) {
                Invoke(nameof(recompute), delay);
                return;
            }

            instance.recomputeBaseRooms(GetComponent<BaseRoot>());
            this.destroy(false);
        }
    }

    private class RoomUpdateQueue : MonoBehaviour {
        private void recompute() {
            if (!DIHooks.IsWorldLoaded()) {
                Invoke(nameof(recompute), 0.5F);
                return;
            }
    ...
    private class DecoRatingTracker {
        internal float totalRating { get; private set; }
        private readonly Dictionary<string, float> totalContribution = new();
        private readonly CountMap<string> objectCount = new();
        ...
        internal void addObject(string id, GameObject go, float deco, bool debug, int repLimit = 2) {
            //start penalizing on the third by default
            if (string.IsNullOrEmpty(id))
                return;
            var hasc = objectCount[id];
            if (deco > 0 && hasc > 0) {
                deco *= getRepeatedDecoPenaltyFactor(hasc + 1, repLimit);
            }

            totalRating += deco;
            var has = totalContribution.ContainsKey(id) ? totalContribution[id] : 0;
        ...
        private float getRepeatedDecoPenaltyFactor(int idx, int safe) {
            //idx being "this is the Nth" (up to 'safe' is unpenalized)
            return idx <= safe
                ? 1
                : 1F / (1F + 0.5F * (idx - safe)); //67% for 3rd, 50% for 4th, 40% for 5th, 33% for 6th, etc
        }
    }
    ...
    internal class RoomTypeTracker : MonoBehaviour {
        ...
        public bool justComputed => DayNightCycle.main.timePassedAsFloat - lastComputeTime <= 0.5F;
            ...
            switch (roomType) {
                case RoomTypes.STORAGE:
                    if (lockers.Contains(prefab.ClassId)) {
                        var sc = prefab.GetComponent<StorageContainer>();
                        var refSc = ObjectUtil.lookupPrefab(prefab.ClassId).GetComponent<StorageContainer>();
                ...
                case RoomTypes.ACU:
                    if (prefab.ClassId == ACU_PREFAB) {
                        var wp = prefab.GetComponent<WaterPark>();
                        var refWp = ObjectUtil.lookupPrefab(prefab.ClassId).GetComponent<WaterPark>();
