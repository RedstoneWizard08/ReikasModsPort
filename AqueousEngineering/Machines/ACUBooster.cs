...
public class ACUBooster : CustomMachine<ACUBoosterLogic> {
    ...
    internal static readonly Dictionary<string, ACUFuel> fuels = new();
    ...
    public override void initializeMachine(GameObject go) {
        ...
        var mdl = go.setModel(
            "model",
            ObjectUtil.lookupPrefab("cdade216-3d4d-4adf-901c-3a91fb3b88c4")
                .getChildObject("model/submarine_centrifuge_base")
        );
        ...
        var lgc = go.GetComponent<ACUBoosterLogic>();
...
public class ACUBoosterLogic : CustomMachineLogic {
    ...
    protected override void updateEntity(float seconds) {
        ...
        if (connectedACU && consumePower(ACUBooster.POWER_COST * seconds)) {
            ...
            if (fuel != null) {
                ...
                var hook = connectedACU.GetComponent<AcuCallbackSystem.AcuCallback>();
                if (hook) {
                    hook.Boost(fuel);
    ...
    private ACUFuel tryFindFuel() {
        foreach (var f in ACUBooster.fuels.Values) {
