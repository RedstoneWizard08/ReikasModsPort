...
public class BaseDrillableGrinder : CustomMachine<BaseDrillableGrinderLogic> {
    ...
    internal static readonly Dictionary<TechType, UncraftingRecipe> uncraftingList = new();
    public static readonly Dictionary<TechType, float> uncraftingIngredientRatios = new();
    public static readonly HashSet<TechCategory> uncraftableCategories = [];
    public static readonly Dictionary<TechType, bool> uncraftabilityFlags = new();
    ...
    [SetsRequiredMembers]
    public BaseDrillableGrinder(XMLLocale.LocaleEntry e) : base(
        e.key,
        e.name,
        e.desc,
        "51eba507-317c-46bf-adde-4459dc8e002e"
    ) {
    ...
    public class UncraftingRecipe {
        ...
        public readonly Dictionary<TechType, float> directYields = new();
        public readonly Dictionary<TechType, float> yields = new();
        ...
        internal void buildRecursiveYields() {
            ...
            foreach (var kvp in directYields) {
                //SNUtil.log("Found recipe for "+kvp.Key.AsString()+": "+BaseDrillableGrinder.uncraftingList.ContainsKey(kvp.Key));
                if (uncraftingList.ContainsKey(kvp.Key)) {
                    var rec = uncraftingList[kvp.Key];
                    if (!rec.isFinalized) {
                        rec.buildRecursiveYields();
                    }

                    foreach (var kvp2 in rec.yields) {
        ...
        private static float getDefaultYield(TechType tt, int amt) {
            var ratio = 0.9F;
            RecipeUtil.getRecipeCategory(tt, out var grp, out var cat);
    ...
    public override void initializeMachine(GameObject go) {
        ...
        var lgc = go.GetComponent<BaseDrillableGrinderLogic>();
...
public class BaseDrillableGrinderLogic : CustomMachineLogic, DIHooks.IStasisReactant {
    ...
    private readonly List<GameObject> drills = [];
    ...
    internal static readonly SoundManager.SoundData jammedSound = SoundManager.registerSound(
        AqueousEngineeringMod.modDLL,
        "crusherjam",
        "Sounds/crusherjam.ogg",
        SoundManager.soundMode3D
    );
    ...
    protected override void updateEntity(float seconds) {
        isReady = !GameModeUtils.RequiresPower() || consumePower(
            (isGrinding ? BaseDrillableGrinder.POWER_COST_ACTIVE : BaseDrillableGrinder.POWER_COST) * seconds
        );
        if (!isReady) {
            freeze(2);
        }
        ...
        if (!aoe) {
            var go = new GameObject("AoEHolder");
        ...
        if (drills.Count != 3) {
            ...
            for (var i = 0; i < 3; i++) {
                ...
                var turbine = air.getChildObject("model/_pipes_floating_air_intake_turbine_geo").clone();
    ...
    internal void OnTriggerStay(Collider other) {
        if (!other.isTrigger && isReady && shutdownCooldown <= 0) {
            var sound = workingSound;
            var loopTime = 3.25F;
            var d = other.gameObject.FindAncestor<Drillable>();
            //SNUtil.writeToChat("Drillable: "+d);
            if (d && d.resources.Length > 0) {
                var s = other.gameObject.FindAncestor<SpecialDrillable>();
                if (s && !s.allowAutomatedGrinding())
                    return;
                isGrinding = true;
                var tt = d.resources[0].techType;
                ...
                if (cg) {
                    isGrinding = true;
                    var flag = cg.isRecyclable();
                    if (flag) {
                        var n = UnityEngine.Random.Range(cg.numberToYieldMin, cg.numberToYieldMax + 1);
                        for (var i = 0; i < n; i++) {
                            //CustomGrindableResult cres = new CustomGrindableResult(cg);
                            //cg.resourceChoice.Invoke(cres);
                            //GameObject drop = cres.drop;
                            var drop = cg.chooseRandomResource(); //.Invoke(cg);
                            if (drop) {
                    ...
                    isGrinding = true;
                    var r = BaseDrillableGrinder.uncraftingList[tt];
                    foreach (var pp2 in pp.GetComponentsInChildren<Pickupable>()) { //child items
                        if (pp2 && pp2 != pp) {
                            pp2.Drop();
                        }
                    }
    ...
    private void drill(Drillable d, TechType tt) {
        var num = 0f;
        for (var i = 0; i < d.health.Length; i++)
            num += d.health[i];
        var zero = Vector3.zero;
        var num2 = d.FindClosestMesh(aoe.transform.position, out zero);
        d.timeLastDrilled = Time.time;
        if (num > 0) {
            var num3 = d.health[num2];
            ...
            if (num3 > 0f && d.health[num2] <= 0f) {
                ...
                //     var amt = UnityEngine.Random.Range(d.minResourcesToSpawn, d.maxResourcesToSpawn);
                //     for (var i = 0; i < amt; i++) {
                //         GameObject drop = d.ChooseRandomResource();
                //         if (drop) {
                //             var res = new DrillableGrindingResult(this, tt, d, drop);
                //             doDrop(res);
                //         }
                //     }
                // }
            ...
            if (num <= 0f) {
                ...
                if (d.deleteWhenDrilled) {
                    var time = d.lootPinataOnSpawn ? 6f : 0f;
                    d.Invoke(nameof(Drillable.DestroySelf), time);
                }
            }
        }
    }
    ...
    private void doDrop(DrillableGrindingResult res) {
        ...
        if (res.drop) {
            for (var a = 0; a < res.dropCount; a++) {
                var use = res.drop.clone();
                use.SetActive(true);
                ...
                var rb = use.GetComponent<Rigidbody>();
...
public class DrillableGrindingResult {
    ...
    public override string ToString() {
        return
            $"[DrillableGrindingResult Grinder={grinder.transform.position}, MaterialTech={materialTech.AsString()}, Drillable={drillable}, OriginalDrop={originalDrop}, Drop={drop}, DropCount={dropCount}]";
    }
}
