...
public class WirelessCharger : CustomMachine<WirelessChargerLogic> {
    ...
    [SetsRequiredMembers]
    public WirelessCharger(XMLLocale.LocaleEntry e) : base(
        e.key,
        e.name,
        e.desc,
        ...
        addIngredient(TechType.SeamothElectricalDefense, 1);
    }
    ...
    public override void initializeMachine(GameObject go) {
        ...
        var mdl = go.setModel(
            "Power_Transmitter",
            ObjectUtil.lookupPrefab("67744b32-93c2-4aba-8a18-ffb87204a8eb").getChildObject("model/LED_light")
        );
        ...
        */
        var lgc = go.GetComponent<WirelessChargerLogic>();
...
public class WirelessChargerLogic : CustomMachineLogic {
    ...
    private static readonly Color noPlayerInRangeColor = new(1, 0, 0);
    private static readonly Color workingColor = new(0, 1, 0);
    private static readonly Color everythingChargedColor = new(0, 0.5F, 1);
    private static readonly Color noToolsColor = new(1, 0, 1);
    ...
    protected override void updateEntity(float seconds) {
        var c = noPlayerInRangeColor;
        if (sub) {
            var ep = Player.main;
            if (ep) {
                var dist = (ep.transform.position - transform.position).magnitude;
                if (dist <= MAX_RANGE) {
                    var eff = getEfficiency(dist);

                    c = Color.Lerp(noPlayerInRangeColor, workingColor, eff);

                    float wanted = 0;
                    var dict = new Dictionary<EnergyMixin, float>();

                    foreach (var e in InventoryUtil.getAllHeldChargeables()) {
                        var ib = e.battery;
                        var max = ib != null ? ib.capacity : e.maxEnergy;
                        var space = max - (ib != null ? ib.charge : e.energy);
                    ...
                    } else if (consumePower(wanted)) {
                        var frac = powerConsumedLastAttempt / wanted;
