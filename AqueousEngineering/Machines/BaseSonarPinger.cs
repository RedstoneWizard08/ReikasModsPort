...
public class BaseSonarPinger : CustomMachine<BaseSonarPingerLogic> {
    ...
    [SetsRequiredMembers]
    public BaseSonarPinger(XMLLocale.LocaleEntry e) : base(
        e.key,
        e.name,
        e.desc,
    ...
    public override void initializeMachine(GameObject go) {
        ...
        var lgc = go.GetComponent<BaseSonarPingerLogic>();
...
public class BaseSonarPingerLogic : ToggleableMachineBase {
    ...
    private void ping(float time) {
        if (consumePower(BaseSonarPinger.POWER_COST)) {
            ...
            SoundManager.playSoundAt(
                SoundManager.buildSound("event:/sub/cyclops/sonar"),
                Player.main.transform.position,
                false,
                BaseSonarPinger.MAX_RANGE,
                4
            );
        }
    }
    ...
    private bool isInAppropriateLocation() {
        var p1 = Player.main.transform.position;
    ...
    protected override void updateEntity(float seconds) {
        ...
        if (!isEnabled)
            return;
        if (rotator && sub) {
            var angs = rotator.transform.localEulerAngles;
            angs.y += 90 * seconds;
