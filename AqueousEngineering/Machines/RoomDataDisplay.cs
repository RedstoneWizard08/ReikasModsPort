...
public class RoomDataDisplay : CustomMachine<RoomDataDisplayLogic> {
    [SetsRequiredMembers]
    public RoomDataDisplay(XMLLocale.LocaleEntry e) : base(
        e.key,
        e.name,
        e.desc,
        "b343166e-3a17-4a1c-85d1-05dee8ec1575"
    ) {
    ...
    public override void initializeMachine(GameObject go) {
        ...
        var ctr = go.EnsureComponent<Constructable>(); /*
...
public class RoomDataDisplayLogic : CustomMachineLogic {
    ...
    protected override void updateEntity(float seconds) {
        ...
        var type = BaseRoomSpecializationSystem.instance.getSavedType(this, out var deco, out var decoThresh);
        var name = AqueousEngineeringMod.roomLocale
            .getEntry(Enum.GetName(typeof(BaseRoomSpecializationSystem.RoomTypes), type)).name;
        var put = name + " (" + deco.ToString("0.00") + ")";
        field.text = put;
