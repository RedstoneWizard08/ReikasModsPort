...
public class AmpeelAntenna : CustomMachine<AmpeelAntennaLogic> {
    ...
    [SetsRequiredMembers]
    public AmpeelAntenna(XMLLocale.LocaleEntry e) : base(
        e.key,
        e.name,
        e.desc,
    ...
    public override void initializeMachine(GameObject go) {
        ...
        var n = 15;
        for (var i = pi.Length; i < n; i++) {
            var fin = ObjectUtil.createWorldObject("cf522a95-3038-4759-a53c-8dad1242c8ed");
            fin.convertToModel();
            ...
            fin.transform.localPosition =
                new Vector3(-0.015F, 0, -0.75F + 1.5F * i / n); //Vector3.zero+i*go.transform.right*0.25F;
            RenderUtil.swapTextures(
                AqueousEngineeringMod.modDLL,
                fin.GetComponentInChildren<Renderer>(),
                "Textures/Machines/AmpeelCoil"
            );
        }

        var lgc = go.GetComponent<AmpeelAntennaLogic>();
    ...
    public static float computeACUValue(WaterPark acu) {
        ...
        foreach (var wp in acu.items) {
            if (wp is WaterParkCreature wpc) {
                var s = wpc.GetComponent<Shocker>();
                var aac = wpc.GetComponent<AmpeelAntennaCreature>();
                if (s && s.liveMixin.IsAlive()) {
                    ampeels += s.liveMixin.GetHealthFraction();
                } else if (aac != null && aac.live && aac.live.IsAlive()) {
                    var f = aac.live.GetHealthFraction();
...
internal class AmpeelCoil : MonoBehaviour {
}
...
public class AmpeelAntennaLogic : CustomMachineLogic {
    ...
    private WaterParkGeometry connectedACUPart;
    ...
    private WaterPark tryFindACU() {
        ...
        foreach (var wp in sub.GetComponentsInChildren<WaterPark>()) {
            if (Mathf.Abs(wp.transform.position.y - transform.position.y) <= 0.5F &&
                Vector3.Distance(wp.transform.position, transform.position) <= 1) {
                return wp;
            }
        }
    ...
    protected override void updateEntity(float seconds) {
        ...
        if (sub && sub.powerRelay.GetPower() < sub.powerRelay.GetMaxPower()) {
            ...
            if (connectedACU) {
                ...
                var set = WorldUtil.getObjectsNearWithComponent<Shocker>(
                ...
                foreach (var c in set) {
                    if (c && c.liveMixin.IsAlive()) {
                        ...
                        if (dd >= AmpeelAntenna.RANGE)
                            continue;
                        toAdd += c.liveMixin.GetHealthFraction() *
                                 (AmpeelAntenna.POWER_GEN - dd * AmpeelAntenna.POWER_FALLOFF);
                    }
                }
            }

            if (toAdd > 0) {
                sub.powerRelay.AddEnergy(seconds * toAdd, out var trash);
            }
        }
    }
}