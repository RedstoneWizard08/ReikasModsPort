...
public class BaseCreatureRepellent : CustomMachine<BaseCreatureRepellentLogic> {
    ...
    internal static readonly Color electricalColor = new(0.2f, 0.9f, 1f, 1f);

    internal static readonly Dictionary<TechType, CreatureEffectivity> effectivityMap = new();
    ...
    private static readonly Vector3[] FIN_POSITIONS = [
        new(0, 0.25F, 0.7F),
        new(0, 0.25F, -0.7F),
        new(0.12F, 0.1F, 0.7F),
        new(0.12F, 0.1F, -0.7F),
    ...
    private static readonly Vector3[] FIN_ROTATIONS = [
        new(-90, 90, 0),
        new(0, 90, 0),
        new(180, 90, 0),
    ];
    ...
    [SetsRequiredMembers]
    public BaseCreatureRepellent(XMLLocale.LocaleEntry e) : base(
        e.key,
        e.name,
        e.desc,
        "4cb154ef-bdb6-4ff4-9107-f378ce21a9b7"
    ) {
    ...
    public override void initializeMachine(GameObject go) {
        ...
        var lgc = go.GetComponent<BaseCreatureRepellentLogic>();
        ...
        for (var i = pi.Length; i < 6; i++) {
            var fin = ObjectUtil.createWorldObject("cf1df719-905c-4385-98da-b638fdfd53f7");
            fin.convertToModel();
            ...
            foreach (var m in r0.materials) {
                ...
                m.SetVector(
                    ShaderPropertyID._BuildParams,
                    new Vector4(1f, 1f, 1.25f, -0.85f)
                ); //last arg is speed, +ve is down
...
internal class CreatureEffectivity {
...
internal class RepellentFin : MonoBehaviour {
    ...
    private void Update() {
        ...
        //body.WakeUp();
        render.materials[0].SetColor(
            ShaderPropertyID._BorderColor,
            Color.Lerp(Color.black, BaseCreatureRepellent.electricalColor, root.getIntensity())
        );
    }
...
public class BaseCreatureRepellentLogic : ToggleableMachineBase {
    ...
    protected override void updateEntity(float seconds) {
        ...
        if (isEnabled) {
            ...
            WorldUtil.getGameObjectsNear(
                transform.position,
                BaseCreatureRepellent.RANGE * BaseCreatureRepellent.maxRangeFactor,
                go => flag |= tryRepel(go, seconds)
            );
            if (Player.main.IsSwimming()) {
                var ddp = Vector3.Distance(Player.main.transform.position, transform.position);
                if (ddp <= 2.5F) {
            ...
            if (flag) {
                if (!consumePower(
                        (BaseCreatureRepellent.POWER_COST_ACTIVE - BaseCreatureRepellent.POWER_COST) * seconds
                    ))
                    cooldown = 5;
            }
        }
    ...
    private bool tryRepel(GameObject go, float seconds) {
        ...
        if (c && c.GetFriend() != Player.main.gameObject &&
            (c.Aggression.Value > 0 || c.GetComponent<AggressiveWhenSeeTarget>())) {
            if (c is GhostLeviatanVoid)
                return false;
            var r = BaseCreatureRepellent.RANGE;
            var r0 = BaseCreatureRepellent.RANGE_INNER;
            ...
            var dd = Vector3.Distance(c.transform.position, transform.position);
            var f = dd <= r0 ? 0.3F : 0.15F;
            if (dd >= r)
                return false;
            ...
            if (c.Scared.Value > 0.5F && dd < r * 0.5F) {
                var vec = transform.position + (c.transform.position - transform.position) * 3;
                c.GetComponent<SwimBehaviour>().SwimTo(vec, 20 * f);
            }

            if (dd <= 4F && c.liveMixin) {
                var dmg = ce != null ? ce.damageFactor : 1;
