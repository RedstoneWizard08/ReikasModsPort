...
public class MoonpoolRotationSystem : SaveSystem.SaveHandler {
    ...
    private static readonly SoundManager.SoundData rotateSound = SoundManager.registerSound(
        AqueousEngineeringMod.modDLL,
        "rotatemoonpool",
        "Sounds/rotatemoonpool.ogg",
        SoundManager.soundMode3D,
        s => { SoundManager.setup3D(s, 64); }
    );

    public static readonly MoonpoolRotationSystem instance = new();
    ...
    public void processObject(GameObject go) {
        if (go.name == MOONPOOL_NAME) {
            var mgr = go.EnsureComponent<MoonpoolRotationManager>();
        }
    }

    public void rotateMoonpool(HolographicControl.HolographicControlTag btn) {
        var mgr = btn.gameObject.FindAncestor<MoonpoolRotationManager>();
        if (mgr) {
            /*
    ...
    public void rebuildBase(Base b) {
        foreach (var mgr in b.GetComponentsInChildren<MoonpoolRotationManager>()) {
            mgr.Invoke(nameof(MoonpoolRotationManager.rebuildButtons), 0.25F); //small delay to allow finish init
        }
    }
    ...
    public class MoonpoolRotationManager : MonoBehaviour {
        ...
        private readonly List<ButtonMount> buttons = [];
        ...
        public void rebuildButtons() {
            ...
            foreach (var geo in gameObject.FindAncestor<BaseRoot>()
                         .GetComponentsInChildren<BaseUpgradeConsoleGeometry>()) {
                if (geo) {
                    var con = geo.GetComponent<BaseDeconstructable>();
                    if (con && con.face != null && con.face.Value.cell.Within(
                            geometry.bounds.mins,
                            geometry.bounds.maxs
                        )) {
    ...
    public class ButtonMount {
        ...
        public ButtonMount(GameObject holder, BaseUpgradeConsoleGeometry geo) {
            ...
            button.transform.parent.position = mount.transform.position + new Vector3(0.0F, 0.4F, 0.0F) +
                                               mount.transform.right * -1.08F;
        ...
        public override string ToString() {
            return (button ? button.controlRef + " @ " + button.transform.position : "NULL BTN") + " in " +
                   (mount ? mount.geometryFace.cell.ToString() : "NULL");
        }
    }
}