...
public static class ESHooks {
    ...
    internal static readonly Color DefaultPingColor = new(1, 186 / 255F, 0, 1);
    ...
    static ESHooks() {
        ...
        if (ExscansionMod.config.getBoolean(ESConfig.ConfigEntries.BASES)) {
            ScannerInjections["2921736c-c898-4213-9615-ea1a72e28178"] =
                ExscansionMod.abandonedBase.markerType; //jellyshroom
            ScannerInjections["42a80cbc-d9fd-49d2-94b3-b5178024b3cb"] =
                ExscansionMod.abandonedBase.markerType; //dgr
            ScannerInjections["99b164ac-dfb4-4a14-b305-8666fa227717"] =
                ExscansionMod.abandonedBase.markerType; //float ctr
            ScannerInjections["569f22e0-274d-49b0-ae5e-21ef0ce907ca"] =
                ExscansionMod.abandonedBase.markerType; //float mtn1
            ScannerInjections["0e394d55-da8c-4b3e-b038-979477ce77c1"] =
                ExscansionMod.abandonedBase.markerType; //float mtn2

            SeabaseReconstruction.WorldgenSeabaseController.onWorldgenSeabaseLoad += bb =>
                bb.gameObject.makeMapRoomScannable(ExscansionMod.abandonedBase.markerType);
        }
        ...
        if (ExscansionMod.config.getBoolean(ESConfig.ConfigEntries.ALIEN)) {
            ...
            ScannerInjections["5cebe7f6-b1ce-4ae0-8008-ccfdac5d5690"] =
                ExscansionMod.alienBase.markerType; //mushroom arch
            ScannerInjections["2a579d8a-5833-4415-acf9-5d7c4d00c65e"] =
                ExscansionMod.alienBase.markerType; //lr arch
            ScannerInjections["beb02a51-139f-4cb1-b7fd-831f8d00e55e"] =
                ExscansionMod.alienBase.markerType; //koosh arch
    ...
    public static void OnSkyApplierSpawn(SkyApplier pk) {
        ...
        if (pi && ScannerInjections.ContainsKey(pi.ClassId)) {
            ...
            if (tt == ExscansionMod.alienBase.Info.TechType &&
                !ExscansionMod.config.getBoolean(ESConfig.ConfigEntries.ALIEN))
                return;
            go.makeMapRoomScannable(tt, true);
        } else if (pi && PrefabData.getPrefab(pi.ClassId) != null &&
                   PrefabData.getPrefab(pi.ClassId).Contains("Coral_reef_jeweled_disk")) {
            go.makeMapRoomScannable(TechType.JeweledDiskPiece);
        } else if (go.isPDA()) {
            go.makeMapRoomScannable(TechType.PDA);
        } else if (ExscansionMod.config.getBoolean(ESConfig.ConfigEntries.BASES) &&
                   go.GetComponent<SeabaseReconstruction.WorldgenSeabaseController>()) {
            go.makeMapRoomScannable(ExscansionMod.abandonedBase.Info.TechType);
        } /*
    ...
    private static bool PlayerCanScanFor(TechType tt) {
        if (Leviathans.Contains(tt))
            return PDAScanner.complete.Contains(tt) ||
                   !ExscansionMod.config.getBoolean(ESConfig.ConfigEntries.LEVISCAN);
        switch (tt) {
            ...
            case TechType.UraniniteCrystal:
                return !ExscansionMod.config.getBoolean(ESConfig.ConfigEntries.RESSCAN) ||
                       PDAScanner.complete.Contains(tt);
            ...
            case TechType.StalkerTooth:
                return !ExscansionMod.config.getBoolean(ESConfig.ConfigEntries.TOOTHSCAN) ||
                       PDAScanner.complete.Contains(TechType.StalkerTooth);
            ...
            case TechType.SpadefishEgg:
                return
                    PDAEncyclopedia.entries.ContainsKey(
                        "UnknownEgg"
                    ); //PDAScanner.complete.Contains(TechType.GenericEgg);
    ...
    */
    ...
    public static void UpdatePingHUDVisibility(uGUI_ResourceTracker gui) { /*
        ...
        if (gui && Player.main && Camera.main)
            gui.gameObject.SetActive(
                Player.main.currentSub == null || !Player.main.currentSub.isBase ||
                Player.main.GetBiomeString() == "observatory" || Vector3.Distance(
    ...
    public static void SetResourcePingType(uGUI_ResourceTracker.Blip blip, TechType type) {
        //default color is 0xFFBA00
        blip.techType = type;
