...
internal class ReefbackJetSuctionTrigger : MonoBehaviour {
    ...
    private void OnTriggerStay(Collider other) {
        if (other.isTrigger || other.gameObject.FindAncestor<Reefback>())
            return;
        var rb = other.gameObject.FindAncestor<Rigidbody>();
        //SNUtil.writeToChat("Jet "+gameObject.name+" sucking "+other);
        if (rb) {
            var dd = transform.position - other.transform.position;
            rb.AddForce(
                (-root.transform.forward * 1.2F + dd.normalized * 15F / dd.sqrMagnitude) * 10F * Time.deltaTime,
