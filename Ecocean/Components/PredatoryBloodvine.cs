...
public class PredatoryBloodvine : MonoBehaviour {
    ...
    private void Update() {
        ...
        var dt = time - eatStart;
        if (dt <= DURATION) {
            //SNUtil.writeToChat("dt: "+dt.ToString("00.000"));
            topScale = dt <= SNAP_TIME
                ? 1 - dt * 0.5F / SNAP_TIME
                : dt > DURATION - RELEASE_TIME
                    ? 0.5F + (dt - RELEASE_START) / RELEASE_TIME * 0.5F
                    : 0.5F;
        } else {
            target = null;
            topScale = 1;
        ...
        if (target && dT > 0) {
            ...
            if (!target.GetComponent<SubRoot>()) {
                var tgt = centerTarget;
                if (target.GetComponent<Vehicle>())
                    tgt += Vector3.up;
                var dd = tgt - target.transform.position;
                if (dd.sqrMagnitude <= 2.25F) {
                    target.transform.position = tgt;
                } else {
                    var rb = target.GetComponent<Rigidbody>();
                    if (rb) {
                        var f = dT * 50F * Mathf.Clamp01(dd.sqrMagnitude * dd.sqrMagnitude / 2);
                        if (target.GetComponent<Vehicle>())
                            f *= 4;
    ...
    internal bool canAttack(GameObject go, out LiveMixin live) {
        ...
        if (gameObject.FindAncestor<Bubble>())
            return false;
        var p = go.FindAncestor<Player>();
        if (gameObject.FindAncestor<Planter>() && (p || go.FindAncestor<Vehicle>()))
            return false;
        if (p && !p.IsSwimming())
            return false;
        var c = go.FindAncestor<Creature>();
        return !(c is ReaperLeviathan) && !(c is GhostLeviathan) && !(c is GhostLeviatanVoid) && !(c is SeaDragon) &&
               !(c is Reefback);
    }
    ...
    internal void trigger(GameObject go) {
        ...
        SoundManager.playSoundAt(
            SoundManager.buildSound("event:/creature/reaper/attack_player_claw"),
            centerTarget,
            false,
            40
        );
