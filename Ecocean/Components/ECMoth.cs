...
public class ECMoth : MonoBehaviour {
    ...
    private readonly LinkedList<float> lightToggles = [];
    ...
    private void Update() {
        ...
        var flashCount = lightToggles.Count;
        ...
        if (heatColumn) {
            ...
            if (time - lastVoidOrganicCollectTime >= 5 && UnityEngine.Random.Range(0F, 1F) < 0.1F &&
                body.velocity.y < -6) { //maxes ~8
                SeamothPlanktonScoop.checkAndTryScoop(
                    seamoth,
                    Time.deltaTime,
                    EcoceanMod.voidOrganic.Info.TechType,
                    out var drop
                );
                if (drop)
    ...
    private float getFlashEffectiveness() {
        var brightness = DayNightCycle.main.GetLightScalar();
