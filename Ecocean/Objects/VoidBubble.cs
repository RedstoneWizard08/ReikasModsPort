...
public class VoidBubble : InteractableSpawnable {
    internal static readonly Color COLOR = new(0.25F, 0F, 1F, 1);
    internal static readonly Color COLOR_ATTACHED = new(0.75F, 0.5F, 1F, 1);

    internal static readonly Simplex3DGenerator densityNoise =
        (Simplex3DGenerator)new Simplex3DGenerator(483181).setFrequency(0.005);
    ...
    public override GameObject GetGameObject() {
        ...
        var g = world.EnsureComponent<VoidBubbleTag>();
        var sc = world.GetComponentInChildren<SphereCollider>();
        sc.radius *= 0.5F;
    ...
    internal void tickSpawner(Player ep, float time, float dT) {
        ...
        var ctr = ep.transform.position;
        var f = (float)(1 + densityNoise.getValue(ctr)) * 0.04F;
        var depth = -ctr.y;
        if (depth > 250) {
        ...
        if (voidBubbleTickEvent != null) {
            var tick = new VoidBubbleSpawnerTick(ep, f);
...
public class VoidBubbleTag : MonoBehaviour {
    ...
    private readonly SimplexNoiseGenerator xSize =
        (SimplexNoiseGenerator)new Simplex1DGenerator(UnityEngine.Random.Range(0, 999999)).setFrequency(0.5F);

    private readonly SimplexNoiseGenerator ySize =
        (SimplexNoiseGenerator)new Simplex1DGenerator(UnityEngine.Random.Range(0, 999999)).setFrequency(0.5F);

    private readonly SimplexNoiseGenerator zSize =
        (SimplexNoiseGenerator)new Simplex1DGenerator(UnityEngine.Random.Range(0, 999999)).setFrequency(0.5F);
    ...
    private void Update() {
        ...
        var color = Color.Lerp(VoidBubble.COLOR, VoidBubble.COLOR_ATTACHED, attachFade);

        age += dT;
        if (attachCooldown > 0)
            attachCooldown -= dT;

        var vec = new Vector3(age, 0, 0);
        var depth = Ocean.main ? Ocean.GetOceanLevel() - 0.8F - transform.position.y : -1000;
        var f = depth > 20 ? 1 : depth / 20F;
        if (stuckTo)
            f *= 0.8F;
        if (fadeDuration > 0)
        ...
        transform.localScale = new Vector3(
            evalSizeNoise(xSize, vec),
            evalSizeNoise(ySize, vec),
            evalSizeNoise(zSize, vec)
        ) * f;
        inner.transform.localPosition = Vector3.zero;
        ...
        if (!stuckTo) {
            if ((transform.position.y < -50 && !VanillaBiomes.Void.IsInBiome(transform.position + Vector3.up * 50)) ||
                UWE.Utils.RaycastIntoSharedBuffer(
    ...
    public void OnCollisionEnter(Collision c) {
        ...
        if (rb) {
            ...
            ObjectUtil.addCyclopsHologramWarning(
                ...
                gameObject,
                Sprite.Create(
                    TextureManager.getTexture(EcoceanMod.modDLL, "Textures/CyclopsVoidBubbleIcon"),
                    new Rect(0, 0, 100, 100),
                    new Vector2(0, 0)
                )
            );
        } else {
            var r = c.collider.GetComponentInParent<VoidBubbleReaction>();
            if (r != null)
