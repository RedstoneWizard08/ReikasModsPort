...
public class HeatColumnBone : CustomPrefab {
    private string basis;

    internal static readonly Dictionary<string, string> basisTable = new();

    public static readonly Dictionary<string, Vector3> boneProps = new() {
    ...
    internal HeatColumnBone(string s) : base("HeatColumnBone_" + s, "", "") {
        basis = s;
        AddOnRegister(() => { basisTable[Info.ClassID] = basis; });
...
public class HeatColumnObject : MonoBehaviour {
    ...
    protected float speedFactor = Random.Range(0.5F, 1.5F);
    protected float scaleFactor = Random.Range(0.5F, 1F);
