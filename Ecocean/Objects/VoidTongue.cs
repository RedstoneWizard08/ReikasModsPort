...
public class VoidTongue : CustomPrefab {
    ...
    public GameObject GetGameObject() {
        ...
        var g = world.EnsureComponent<VoidTongueTag>();
...
public class VoidTongueTag : MonoBehaviour {
    ...
    private static readonly SoundManager.SoundData grabSound = SoundManager.registerSound(
        EcoceanMod.modDLL,
        "tonguegrab",
        "Sounds/tonguegrab.ogg",
        SoundManager.soundMode3D,
        s => { SoundManager.setup3D(s, 120); }
    );

    private static readonly SoundManager.SoundData pullSound = SoundManager.registerSound(
        EcoceanMod.modDLL,
        "tonguepull",
        "Sounds/tonguepull.ogg",
        SoundManager.soundMode3D,
        s => { SoundManager.setup3D(s, 120); }
    );
    ...
    private void Update() {
        ...
        if (!tip) {
            ...
            if (!tip) {
                ...
                light = tip.addLight();
                var sc = tip.EnsureComponent<SphereCollider>();
                sc.radius = 1;
        ...
        var tt = getCollider(getPriorityTarget());
        ...
        if (stuckTo) {
            ...
            if (currentPullingSound != null && currentPullingSound.Value.hasHandle()) {
                var attr = stuckTo.transform.position.To3DAttributes();
                currentPullingSound.Value.set3DAttributes(ref attr.position, ref attr.velocity);
            }
            ...
            if (stuckCyclops) {
                ...
                stuckTo.AddForce(
                    (transform.position - stuckTo.position).normalized * dT * 5000,
                ...
                var ch = stuckTo.GetComponent<CrushDamage>();
                if (ch)
            ...
            if (stuckCreature)
                VoidGhostLeviathansSpawner.main.timeNextSpawn = Mathf.Max(
                    Time.time + 90,
                    VoidGhostLeviathansSpawner.main.timeNextSpawn
                );
            ...
            if (length > 600 || (tt && tip.transform.position.y >= tt.transform.position.y + 100)) {
                ...
                var targetRotation = Quaternion.LookRotation(
                    transform.up,
                    Vector3.up
                ); //MathUtil.unitVecToRotation(-transform.up);
        ...
        var unitVec = (transform.position - tt.position).normalized;
        ...
        var delay = UnityEngine.Random.Range(8F, 15F);
        if (stuckCyclops) {
    ...
    private void doKill() {
        if (stuckCyclops) { /*
            ...
            var lv = stuckTo.GetComponentInChildren<LiveMixin>();
            if (lv) {
                lv.Kill();
    ...
    private Transform getPriorityTarget() {
        ...
        foreach (var go in VoidGhostLeviathansSpawner.main.spawnedCreatures) {
            if (go && go.activeInHierarchy && go.GetComponent<LiveMixin>().IsAlive() &&
                Vector3.Distance(ep.transform.position, go.transform.position) <= 120)
                return go.transform;
        }

        if (ep.transform.position.y <= -1800)
            return ep.transform;
        foreach (var sub in FindObjectsOfType<SubRoot>()) {
            if (sub && sub.gameObject.activeInHierarchy && sub.isCyclops && !sub.subDestroyed &&
                VanillaBiomes.Void.IsInBiome(sub.transform.position))
                return sub.transform;
        }

        foreach (var v in FindObjectsOfType<Vehicle>()) {
            if (v && v.gameObject.activeInHierarchy && v.liveMixin.IsAlive() &&
                VanillaBiomes.Void.IsInBiome(v.transform.position))
                return v.transform;
        }
    ...
    public void Disconnect() {
        if (stuckTo != null) {
            ...
            var hud = stuckTo.GetComponentInChildren<CyclopsHolographicHUD>();
            if (hud) {
    ...
    public void grab(Rigidbody rb) {
        if (stuckTo || !isGrabbing)
            return;
        if (rb) {
            var p = rb.GetComponentInChildren<Player>();
            bool pda = p;
            if (p)
            ...
            var v = Util.GetComponentInHierarchy<Vehicle>(rb.gameObject);
            //SNUtil.writeToChat("V: "+v);
            SoundManager.playSoundAt(grabSound, rb.transform.position, false, -1, 2);
            if (stuckCyclops) {
                stuckCyclops.EndSubShielded();
                SoundManager.playSoundAt(
                    SoundManager.buildSound("event:/sub/cyclops/impact_solid_hard"),
                ...
                SoundManager.playSoundAt(
                    SoundManager.buildSound("event:/sub/seamoth/impact_solid_hard"),
                    rb.transform.position
                );
            ...
            ObjectUtil.addCyclopsHologramWarning(
                ...
                tip,
                Sprite.Create(
                    TextureManager.getTexture(EcoceanMod.modDLL, "Textures/CyclopsVoidTongueIcon"),
...
internal class VoidTongueGrabDetector : MonoBehaviour {
    ...
    public void OnTriggerEnter(Collider other) {
        ...
        var rb = hit ? hit.GetComponentInChildren<Rigidbody>() : null;
