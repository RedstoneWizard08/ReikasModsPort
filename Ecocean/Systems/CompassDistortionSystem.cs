...
public class CompassDistortionSystem {
    public static readonly CompassDistortionSystem instance = new();

    private readonly List<LocalDistortion> localEffects = [];
    ...
    internal void tick(float time, float dT) {
        var dE = time - lastEMPHitTime;
        if (dE < empSpinTime) {
            var f = dE / empSpinTime;
            var speed = f < 0.5F ? 1F : f < 0.75F ? 1F - (f - 0.5F) : 0.75F - (f - 0.75F) * 3;
            if (speed > 0)
            ...
            empDisplacement = Math.Abs(empDisplacement) >= 1
                ? Mathf.Min(empDisplacement % 360F + 30 * dT, 360) % 360F
                : 0;
    ...
    public float getTotalDisplacement(Vector3 pos) {
        var ret = empDisplacement;
        getOrCreateGlobalNoise();
        var time = DayNightCycle.main.timePassedAsFloat;
        var noise = (float)globalDistortionCutoff.getValue(new Vector3(time, 0, 0));
        var depth = Mathf.Max(0, -pos.y);
        ...
        if (noise > 0) {
            var globalIntensity =
                0.5F + 0.5F *
                Mathf.Clamp01(
                    EcoceanMod.config.getFloat(ECConfig.ConfigEntries.GLOBALCOMPASS)
                ); //config is 0-1 for "how often"
        ...
        foreach (var ld in localEffects) {
            if (ld.isInRange(pos)) {
    ...
    public class PointRadiusDistortion : LocalDistortion {
        ...
        public PointRadiusDistortion(Vector3 pos, float r, float a, float f) : base(
            a,
            f,
            (pos.x.GetHashCode() | (pos.z.GetHashCode() << 32)) ^ (pos.y * 20F).GetHashCode()
        ) {
            position = pos;
            radius = r;
        }
