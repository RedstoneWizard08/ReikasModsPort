...
public class VanillaFlora : PrefabReference, Flora {
	...
	private static readonly Dictionary<string, VanillaFlora> lookup = new();
	...
	public static readonly List<string> BLOOD_ROOT_PROP = [
	...
	public static readonly List<string> BLOOD_ROOT_FERTILE = [
	...
	private static readonly Dictionary<string, VanillaFlora> names = new();
	...
	private static void populateNames() {
		if (names.Count == 0) {
			foreach (var f in typeof(VanillaFlora).GetFields()) {
				if (f.IsStatic && f.FieldType == typeof(VanillaFlora)) {
					var vf = (VanillaFlora)f.GetValue(null);
					names[f.Name] = vf;
					vf.name = f.Name;
				}
			}
	...
	private readonly List<string> prefabsLit = [];
	...
	private readonly List<BiomeBase> biomeSetSurface = [];
	private readonly List<BiomeBase> biomeSetCave = [];
	...
	private VanillaFlora(double y, double ym, params string[] ids) {
		foreach (var id in ids) {
			var path = PrefabData.getPrefab(id);
	...
	public string getRandomPrefab(bool preferLit) {
		var check = preferLit ? prefabsLit : prefabs;
		if (check.Count == 0) {
