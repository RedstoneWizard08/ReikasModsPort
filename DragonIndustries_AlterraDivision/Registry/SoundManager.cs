...
public static class SoundManager {
    ...
    public static SoundData registerPDASound(Assembly a, string id, string path) {
        return registerSound(a, id, path, soundModeStreaming, null, AudioUtils.BusPaths.PDAVoice);
    }

    public static SoundData registerSound(
        Assembly a,
        string id,
        string path,
        MODE m,
        Action<Sound> processing = null,
        string busPath = null
    ) {
        if (a == null)
    ...
    public static void stopSound(string id, bool allowFade = true) {
        if (sounds.ContainsKey(id))
            EventInstance.FMOD_Studio_EventInstance_Stop(
                sounds[id].internalObject.handle,
                allowFade ? FMOD.Studio.STOP_MODE.ALLOWFADEOUT : FMOD.Studio.STOP_MODE.IMMEDIATE
            );
    }
    ...
    public static Channel? playSoundAt(
        SoundData snd,
        Vector3 position,
        bool queue = false,
        float distanceFalloff = 16F,
        ...
        if (queue) {
            PDASounds.queue.PlayQueued(
                snd.asset
            ); //PDASounds.queue.PlayQueued(path, "subtitle");//PDASounds.queue.PlayQueued(ass);
            return null;
        }
    ...
    public struct SoundData {
        ...
        public readonly string soundBus;

        internal SoundData(string i, FMODAsset a, Sound s, float len, string b) {
            id = i;
            asset = a;
            internalObject = s;
            length = len;
