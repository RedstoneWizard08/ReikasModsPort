...
public class PrecursorStoryConsole : CustomPrefab {
    ...
    public PrecursorStoryConsole(XMLLocale.LocaleEntry e, string goal = null) : base(e.key, e.name, e.desc) {
        ...
        AddOnRegister(() => { prefabTable[Info.ClassID] = this; });
...
public class StoryConsoleTag : MonoBehaviour {
    ...
    private void Update() {
        ...
        if (target) {
            ...
            target.secondaryTooltip = target.enabled
                ? prefab.locale.getString("tooltipSecondary")
                : prefab.locale.getString("tooltipDisabled");
        }
    }
