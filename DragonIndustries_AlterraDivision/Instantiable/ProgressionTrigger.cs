...
public class StoryUntilStoryTrigger : ProgressionTrigger {
    ...
    public StoryUntilStoryTrigger(string key1, string key2) : base(ep =>
        StoryGoalManager.main.completedGoals.Contains(key1) && !StoryGoalManager.main.completedGoals.Contains(key2)
    ) {
        storyKeyFrom = key1;
        storyKeyTo = key2;
    }
...
public class PositionTrigger : ProgressionTrigger {
    ...
    public PositionTrigger(Vector3 pos, float r = 10) :
        base(ep => (ep.transform.position - pos).sqrMagnitude <= r * r) {
        position = pos;
        radius = r;
    }
...
public class TechUnlockEffect : DelayedProgressionEffect {
    ...
    public TechUnlockEffect(TechType tt, float chance = 1, float mind = 0) : base(
        () => unlockTech(tt),
        () => KnownTech.knownTech.Contains(tt),
        chance,
        mind
    ) {
        unlock = tt;
...
public class DelayedEncyclopediaEffect : DelayedProgressionEffect {
    ...
    public DelayedEncyclopediaEffect(PDAManager.PDAPage g, float f, float mind = 0, bool doSound = true) : base(
        () => g.unlock(doSound),
        g.isUnlocked,
        f,
        mind
...
public class DelayedStoryEffect : DelayedProgressionEffect {
    ...
    public DelayedStoryEffect(StoryGoal g, float f, float mind = 0) : base(
        () => StoryGoal.Execute(g.key, g.goalType),
        () => StoryGoalManager.main.completedGoals.Contains(g.key),
        f,
        mind
    ) {
        goal = g;
        if (g != null) goalKey = g.key;
    }

    public override string ToString() {
        return goal == null ? "Story [unknown]" : "Story " + goal.key;
    }
}